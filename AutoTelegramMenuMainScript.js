...
Object.defineProperty(Date.prototype, "format", {
    value:  function(mask, utc) { return dateFormat(this, mask, utc); }
});
    ...
    saveOption(cfgItem, user) {
        ...
        if ((currentValue !== undefined) && (currentValue !== null)) {
            ...
            if (existsState(id) && (getObject(id).common.type === stateType)) {
                ...
                if (existsState(id)) deleteState(id, (error, result) => {
                    if (error) {
        ...
        if (typeOf(backupData, 'object') && backupData.hasOwnProperty('globalConfig') && typeOf(backupData.globalConfig, 'object')) {
            Object.keys(this.globalConfig).forEach(cfgItem => {
                if (backupData.globalConfig.hasOwnProperty(cfgItem) && (backupData.globalConfig[cfgItem] !== undefined) && (backupData.globalConfig[cfgItem] !== null)) {
                    // logs(`setOption(${cfgItem}) = ${JSON.stringify(backupData.globalConfig[cfgItem])}`, l)
                ...
                backupData.perUserConfig.forEach((userConfigData, userId) => {
                    Object.keys(this.globalConfig).forEach(cfgItem => {
                        if ( (! configGlobalOptions.includes(cfgItem)) && userConfigData.hasOwnProperty(cfgItem) && (userConfigData[cfgItem] !== undefined) && (userConfigData[cfgItem] !== null)) {
                            // logs(`setOption(${cfgItem}, ${JSON.stringify({userId})}) = ${JSON.stringify(userConfigData[cfgItem])}`, l)
    ...
    menuGenerateForArray(user, menuItemToProcess) {
        ...
        for(const itemValue of currentArray) {
            ...
            if (isThisLevelAllowModify) {
                ...
                subSubMenuIndex = subMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeConfig, cfgItem, `${currentIndex}.${subMenuIndex}`, subSubMenuIndex, optionScope, subMenuIndex));
            }
            else {
                            ...
                            subMenuItem.submenu = (user, menuItemToProcess) => {
                                ...
                                Object.keys(translationsList).sort().forEach((languageId) => {
                                    ...
                                    if (currentLanguage !== languageId) {
                                        ...
                                        subSubMenuIndex = subMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeConfig, cfgItem, `${currentIndex}.${subMenuIndex}`, subSubMenuIndex, optionScope, languageId));
                                    }
                                    subMenuIndex = subMenu.push(subMenuItem);
                                });
                                    ...
                                    subMenuIndex = subMenu.push({
                                        ...
                                        submenu: (user, menuItemToProcess) => {
                                            ...
                                            if (cachedExistsValue(user, cachedConfigNewLanguageId)) {
                                                const newLanguageId = translationValidateLanguageId(cachedGetValue(user, cachedConfigNewLanguageId));
                            ...
                            subMenuItem.submenu = (user, menuItemToProcess) => {
                                ...
                                currentIntervals.forEach(({id: graphsIntervalId, minutes: graphsIntervalMinutes}, graphsIntervalIndex) => {
                                    ...
                                    if (isThisLevelAllowModify) {
                                        ...
                                        subSubMenuIndex = subMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeConfig, cfgItem, `${currentIndex}.${subMenuIndex}`, subSubMenuIndex, optionScope, graphsIntervalIndex));
                                    }
...
class MenuRoles {
     ...
     * @param {string=} options - The id of the role to delete.
     * @returns {boolean} The result of the delete operation.
     */
    delRole(itemId, options) {
        let result = false;
        if (this.existsId(itemId)) {
     ...
     * @param {boolean=} compiled - defined to be compatible with the child class (`MenuUsers`) method.
    ...
    #menuRuleSetAccessLevel(user, menuItemToProcess) {
        ...
        const
            currentIndex = menuItemToProcess.index !== undefined ? menuItemToProcess.index : '',
            [cmdId, dataType, roleId, ruleIndex] = commandUnpackParams(menuItemToProcess.param),
    ...
    #menuGenerateRoleRulesAddItem(user, menuItemToProcess) {
        ...
        function selectMask(user, menuItemToProcess) {
            const
                currentIndex = menuItemToProcess.index !== undefined ? menuItemToProcess.index : '',
                [cmdId, upperItemId, currentItemId] = commandUnpackParams(menuItemToProcess.param),
        ...
        function menuGenerateItemWithSubMenus(user, menuItemToProcess, currentIndex, parentItemId) {
            const resultItem = {
                ...
                function: (user, menuItemToProcess) => (MenuRoles.#ruleDetails(user, cachedGetValue(user, cachedRolesNewRule))),
                submenu: new Array()
            };
            const
        ...
        subMenuIndex = subMenu.push({
            ...
            function: (user, menuItemToProcess) => (MenuRoles.#ruleDetails(user, savedRule)),
    ...
    #menuGenerateRoleRules(user, menuItemToProcess) {
        ...
        currentRoleRules.forEach(rule => {
            ...
            if ((roleId === rolesDefaultAdmin) || (MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) >= 0)) {
                ...
                if (rule.mask !== rolesMaskAnyItem) subSubId = currentItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeMenuRoleRules, roleId, `${currentIndex}.${subMenuIndex}`, subSubId, subMenuIndex));
            }
        ...
        if ((roleId !== rolesDefaultAdmin) &&  (isNewRole || (MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) < 0))) {
            ...
            if ((isNewRole && currentRoleRules.length) || (JSON.stringify(this.getRules(roleId)) !== JSON.stringify(currentRoleRules))) {
                ...
                subMenuIndex = subMenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeMenuRoles, roleId, `${currentIndex}`, subMenuIndex));
            }
        }
        return subMenu;
    }
...
const
    ...
    translationPrimaryStateId = 'primaryState',
    translationCoreId = 'core',
    translationTopItems = [translationCoreId, idFunctions, idDestinations, idSimpleReports],
    translationUpdateModes = ['replace', 'overwrite', 'enrich', 'template'];
 ...
 */
function translationValidateLanguageId(languageId){
    let langId = '';
    try {
...
function translationsLoad() {
    ...
    $(`state[id=${prefixTranslationStates}.*]`).each( (translationId) => {
        const
            ...
            canonicalLang = translationValidateLanguageId(languageId);
...
function translationsLoadLocalesFromRepository(languageId, callback) {
    ...
    if (languageId && (languageId !== doAll)) {
        languageId = translationValidateLanguageId(languageId);
    }
    else {
        languageId = doAll;
    }
        ...
        .then(function (response) {
            if (response && response.data) {
                let
                    localesLinks = {},
                    parsedLocale;
                while (parsedLocale = translationLocalesExtractRegExp.exec(response.data)) {
            ...
            if (error) {
                ...
                Object.keys(locales).forEach(languageId => {
                    if (typeOf(locales[languageId], 'object') && translationsCheckAndCacheUploadedFile(cachedCommonId, '', '', '', locales[languageId])) {
                        ...
                        if (newTranslation && newTranslation.hasOwnProperty('translation') && typeOf(newTranslation['translation'], 'object') && (Math.abs(translationCompareVersion(newTranslation['version'])) < 10)) {
                            translationsList[languageId] = newTranslation['translation'];
                        }
                    }
                });
                translationsSave();
 ...
 */
function translationCompareVersion(versionToCompare) {
    const
        ...
        [translationCurrentVersionMajor, translationCurrentVersionMinor] = translationVersion.split('.'),
...
function translationsProcessLanguageUpdate(user, translationPart, translationUpdateMode) {
    ...
    let result = '';
    if (cachedExistsValue(user, cachedTranslationToUpload)) {
        const
            translationInputFull = cachedGetValue(user, cachedTranslationToUpload),
            translationInputLanguage = translationInputFull.hasOwnProperty('language') ? translationValidateLanguageId(translationInputFull.language) : '',
            translationInputVersionCompare = translationCompareVersion(translationInputFull['version']);
        if (translationInputFull.hasOwnProperty('translation') && typeOf(translationInputFull['translation'], 'object') && (Math.abs(translationInputVersionCompare) < 10)) {
            const
                translationPossibleUpdateModes = translationInputVersionCompare ? ['overwrite'] : translationUpdateModes,
                translationCurrentParts = translationPart === doAll ? translationTopItems : [translationPart],
            ...
            if (translationPossibleUpdateModes.includes(translationUpdateMode)) {
                const translationInput = translationInputFull['translation'];
                translationCurrentParts.forEach((translationPart) => {
                    if (translationInput.hasOwnProperty(translationPart)) {
...
function translationsItemGet(user, translationId) {
    ...
    // logs(`translationId = ${translationId}`, l)
    const
    ...
    if (translationId && shortId && currentTranslation) {
        ...
        else if (((translationId.indexOf(translationCommonFunctionsAttributesPrefix) !== 0) && (currentTranslation[shortId].indexOf(translationCommonFunctionsAttributesPrefix) === 0))
            || (idPrefix && (translationId.indexOf(translationCommonFunctionsAttributesPrefix) !== 0) && (currentTranslation[shortId].indexOf(translationCommonFunctionsAttributesPrefix) === 0))) {
 ...
 */
 function translationsItemCoreGet(user, translationId) {
    return translationsItemGet(user, `${translationCoreId}.${translationId}`);
 }
    ...
    if ((typeof(functionId) === 'string') && (functionsList.hasOwnProperty(functionId))) {
        if (! typeOf(destinationId, 'string') || (destinationId && typeOf(destinationId, 'string') && destinationsList.hasOwnProperty(destinationId))) {
            if (object && (((typeof(object) === 'object') && object.hasOwnProperty('_id')) || (typeof(object) === 'string'))) {
                ...
                translationId = `${isCommon ? translationCommonFunctionsAttributesPrefix : `${idFunctions}.${funcEnum}.${functionId}`}.${destinationId ? `${idDestinations}.${destinationId}.${objectId.split('.').join('_')}` : objectId.replace(`${prefixId}.`, '').split('.').join('_')}`;
            }
        }
    }
...
function translationsGetObjectName(user, object, functionId, destinationId, isCommon) {
    ...
    // logs(`object = ${object}, translationId = ${translationId}`, l);
    if (translationId) {
...
function translationsGetEnumId(user, enumerationType, enumId, enumNameDeclinationKey) {
    if (enumId) {
        ...
        if (enumerationType && (enumerationsList.hasOwnProperty(enumerationType)) && enumerationsList[enumerationType].list.hasOwnProperty(enumId)) {
            const enumPrefix = `${enumerationsList[enumerationType].id}.${enumerationsList[enumerationType].list[enumId].enum}`;
            if ((enumNameDeclinationKey === undefined) || (enumNameDeclinationKey === null)) enumNameDeclinationKey = enumerationNamesMain;
            return `${enumPrefix}.${enumId.replace('.', '_')}.${enumerationNamesTranslationIdPrefix}.${enumNameDeclinationKey}`;
        }
    }
    return '';
}
...
function translationsGetPartName(user, translationPartId) {
    const translationPartPrefix = [translationCoreId, doAll].includes(translationPartId) ? 'translation' : ''
        ...
        try {
            ...
            if (translationObject || (translationFileSize && Number(translationFileSize) === 0) || (translationFileSize && Number(translationFileSize) && (fileStats.size === Number(translationFileSize)))) {
                ...
                try {
                    ...
                    const currentLanguage = translationValidateLanguageId(inputTranslation.language);
                    translationFileName = translationFileName ? translationFileName : `locale_${currentLanguage}.json`;
                    // logs(`inputTranslation = ${JSON.stringify(inputTranslation, null, 2)}`, l);
                    // logs(`${(inputTranslation.type === translationType)}, ${(inputTranslation.version === translationVersion)}, ${(currentLanguage)}, ${inputTranslation.translation}`, l)
                    if ((inputTranslation.type === translationType) && (inputTranslation.version === translationVersion)
                        && (currentLanguage) && inputTranslation.translation) {
                            cachedSetValue(user, cachedTranslationToUpload, inputTranslation);
...
function translationsUploadMenuGenerate(user, menuItemToProcess) {
    ...
    let
        subMenuIndex = 0,
        subMenu = [];
    if (isCurrentAccessLevelAllowModify && cachedExistsValue(user, cachedTranslationToUpload)) {
        const
            inputTranslation = cachedGetValue(user, cachedTranslationToUpload),
            currentLanguage = inputTranslation ? translationValidateLanguageId(inputTranslation.language) : '',
            currentVersion =  inputTranslation ? inputTranslation.version : '';
        if (currentLanguage && inputTranslation.translation) {
            const translationParts = [...translationTopItems, doAll];
            translationParts.forEach(translationPart => {
                if (inputTranslation.translation.hasOwnProperty(translationPart) || (translationPart === doAll)) {
                    const
                        translationPartName = translationsGetPartName(user, translationPart),
                        subMenuItem = {
                            index: `${currentIndex}.${subMenuIndex}`,
                            name: `${translationsItemMenuGet(user, 'ItemsProcess')} ${translationPartName}`,
                            icon: iconItemApply,
                            group: cmdItemsProcess,
                            param: commandsPackParams(cmdEmptyCommand, dataTypeTranslation, translationPart),
                            function: translationsUploadMenuItemDetails,
                            submenu: new Array()
                        };
                    translationUpdateModes.forEach((translationUpdateMode, translationUpdateModeIndex) => {
                        subMenuItem.submenu.push({
                            index: `${currentIndex}.${subMenuIndex}.${translationUpdateModeIndex}`,
                            name: translationsItemMenuGet(user, 'translation', translationUpdateMode),
                            icon: iconItemApply,
                            group: cmdItemsProcess,
                            function: translationsUploadMenuItemDetails,
                            param: commandsPackParams(cmdItemsProcess, dataTypeTranslation, translationPart, translationUpdateMode),
                            submenu: []
                        });
...
function translationsUploadMenuItemDetails(user, menuItemToProcess) {
    const
        [cmdId, currentType, currentPart, currentMode] = commandUnpackParams(menuItemToProcess.param),
        currentItemDetailsList = [];
    if (cachedExistsValue(user, cachedTranslationToUpload)) {
        const
            inputTranslation = cachedGetValue(user, cachedTranslationToUpload),
            currentLanguage = translationValidateLanguageId(inputTranslation.language),
            currentVersion = inputTranslation.version;
        ...
        if (currentPart && (currentPart.indexOf(doUpload) !== 0)) {
            currentItemDetailsList.push({label: translationsItemTextGet(user, 'translationPart'), valueString:translationsGetPartName(user, currentPart)});
        }
    }
...
function translationsBasicItemsMenuGenerate(user, menuItemToProcess) {
    ...
    Object.keys(currentTranslation)
        .filter((key) => (! key.includes('.')) && (key.indexOf(translationType) === 0))
        .sort()
        .forEach((key) => {
            ...
            if ((! key.includes('.')) && (key.indexOf(translationType) === 0)) {
                let
                    ...
                    if (isCurrentAccessLevelAllowModify) {
                        subSubMenuIndex = subMenuItem.submenu.push(menuRenameItemMenuItemGenerate(user,`${currentIndex}.${subMenuIndex}`, subSubMenuIndex, dataTypeTranslation, key, translationType));
                        subSubMenuIndex = subMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeTranslation, key, `${currentIndex}.${subMenuIndex}`, subSubMenuIndex));
                    }
                    else {
                        subMenuItem.param = cmdNoOperation
                    }
...
function translationsFunctionStatesItemsMenuGenerate(user, menuItemToProcess) {
    const
        ...
        ;
    // logs(`translationType = ${translationType}, currentTranslation = ${JSON.stringify(currentTranslation)}, currentCommonPrefix = ${commonFunctionsAttributesTranslationPrefix}`, l);
    let
        subMenuIndex = 0,
        subMenu = [];
    Object.keys(currentTranslation)
        ...
        .forEach((translationKey, translationKeyIndex) => {
            // logs(`key = ${translationKey}, translationType = ${translationType}`, l);
            ...
            else if (currentValue.indexOf(translationCommonFunctionsAttributesPrefix) === 0) {
                currentValue = `${translationsItemGet(user, `${translationCommonFunctionsAttributesPrefix}.${translationKey}`)} ${iconItemCommon}`;
            }
            let
                subSubMenuIndex = 0,
                subMenuItem = {
                    ...
                    icon: translationType === translationCommonFunctionsAttributesPrefix ? iconItemCommon : '',
                    submenu: new Array()
                };
            if (isCurrentAccessLevelAllowModify) {
                ...
                subSubMenuIndex = subMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeTranslation, translationKey, `${currentIndex}.${translationKeyIndex}`, subSubMenuIndex));
            }
            else {
                subMenuItem.param = cmdNoOperation;
            }
...
function translationsFunctionDeviceItemsMenuGenerate(user, menuItemToProcess) {
    const
        ...
        isCurrentAccessLevelAllowModify = MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) < 0,
        translationType = `${idFunctions}.${menuItemToProcess.id}`,
        currentFunction = menuItemToProcess.funcEnum,
    ...
    // logs(`translationType = ${translationType}, currentTranslation = ${JSON.stringify(currentTranslation)}`, l);
    let subMenu = [];
    Object.keys(currentTranslation)
        ...
        .forEach((translationKey, translationKeyIndex) => {
            const
                subMenuItem = {
                    index: `${currentIndex}.${translationKeyIndex}`,
                    name: `${translationsGetEnumName(user, dataTypeDestination, translationKey, enumerationNamesMain)}`,
                    // text: ` [${currentTranslationId}]`,
                    // icon: currentIcon,\
                    submenu: new Array()
                },
            ...
            Object.keys(currentDevices).forEach((translationDeviceKey, translationDeviceKeyIndex) => {
                // logs(`key = ${translationDeviceKey}, translationType = ${translationType}`, l);
                ...
                if (isCurrentAccessLevelAllowModify) {
                    ...
                    subSubMenuIndex = subSubMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeTranslation, translationType, `${currentIndex}.${translationKeyIndex}.${translationDeviceKeyIndex}`, subSubMenuIndex, translationKeyIndex, translationDeviceKeyIndex));
                }
                else {
...
const
    enumerationNamesMain = 'Main',
    enumerationNamesBasic = 'Basic',
    enumerationNamesMany = 'Many',
    enumerationNamesInside = 'Inside',
    enumerationNamesEnterTo = 'EnterTo',
    enumerationNamesExitFrom = 'ExitFrom',
    enumerationNamesTranslationIdPrefix = 'names',
    enumerationDeviceBasicAttributes = '-ts:lc:-ack:-q:-from:-user',
    ...
    enumerationAccessLevelToShow = 'showAccessLevel',
    enumerationAccessLevelToPress = 'pressAccessLevel',
    enumerationDeviceButtonsAccessLevelAttrs  = [enumerationAccessLevelToShow, enumerationAccessLevelToPress],
    enumerationExcludeForExternal = ['deviceAttributes', 'deviceButtons', 'devicesTranslation', 'deviceAttributesValuesTranslation', 'deviceButtonsValuesTranslation', 'simplifyMenuWithOneDevice', 'showDestNameOnSimplify', 'statesInFolders'],
    enumerationFunctionNotFound = 'noFunction',
    convertButtonToAttribute = 'useButtonAsAttribute',
    ...
    enumerationsList = {
        ...
        [dataTypeDestination]: {
            ...
            defaultObject: {
                ...
                icon: '',
                name: enumerationNamesMain,
                names: [enumerationNamesBasic, enumerationNamesInside, enumerationNamesEnterTo, enumerationNamesExitFrom],
                group: '',
                order : 0,
            }
        },
 ...
 */
function enumerationCompareOrderOfItems(a, b, enumerationType) {
 ...
 */
function enumerationReorderItems(currentEnumeration) {
                ...
                if ((enumerationType === dataTypeFunction) && currentEnumerationList[currentItem].hasOwnProperty('deviceAttributes') && (typeOf(currentEnumerationList[currentItem].deviceAttributes, 'string'))) {
                    ...
                    currentDeviceAttributes.split(':').forEach((deviceAttr, stateIndex) => {
                        ...
                        deviceAttributesList[deviceAttr] = {
                            isEnabled : isEnabled,
                            convertValueCode: "",
                            nameTranslationId: translationsGetObjectId(deviceAttr.split('.').join('_'), currentItem, undefined, enumerationDeviceBasicAttributes.includes(deviceAttr)),
                            order : stateIndex
                        }
                    });
...
function enumerationsUpdateItemName(user, enumerationType, enumerationItemId, enumerationItem, newName, nameDeclinationKey) {
    if (newName && (typeof(newName) === 'string')) {
        ...
        if (enumerationItem.hasOwnProperty('names') &&  (typeOf(enumerationItem.names) === 'array')) {
            ...
            tmpKeys.unshift(enumerationNamesMain);
            if (nameDeclinationKey) {
                ...
                if (translationsItemGet(user, `${translationIdPrefix}${enumerationNamesMain}`) != newName)  {
                    tmpKeys.forEach(nameKey => translationsItemStore(user, `${translationIdPrefix}${nameKey}`, nameKey === enumerationNamesMain ? newName : newName.toLowerCase()));
                }
            }
        }
        else {
            if (translationsItemGet(user, `${translationIdPrefix}${enumerationNamesMain}`) != newName) translationsItemStore(user, `${translationIdPrefix}${enumerationNamesMain}`, newName);
        }
    }
}
...
function enumerationItemGroupsMenuGenerate(user, menuItemToProcess) {
    ...
    const
        currentIndex = menuItemToProcess.index !== undefined ? menuItemToProcess.index : '',
        [cmdId, dataType, currentItem, dataTypeExtraId] = commandUnpackParams(menuItemToProcess.param),
        currentEnumeration = enumerationGetList(dataType, dataTypeExtraId),
 ...
 * @returns {object[]}  Newly generated submenu.
 */
function enumerationItemMenuGenerate(user, menuItemToProcess) {
    logs(`menuItemToProcess = ${JSON.stringify(menuItemToProcess)}`);
    let
        subMenuIndex = 0,
        subMenu = [];
    const
        ...
        currentAccessLevel = menuItemToProcess.accessLevel,
        isCurrentAccessLevelAllowModify = MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) < 0,
        [cmdId, dataType, currentItem, paramToSkip, dataTypeExtraId, otherParams] = commandUnpackParams(menuItemToProcess.param),
        currentEnumeration = enumerationGetList(dataType, dataTypeExtraId);
    const
    ...
    for (let enumerationItemAttr of enumerationItemAttrs) {
        if (currentEnumerationItem.isExternal && enumerationExcludeForExternal.includes(enumerationItemAttr)) {
            continue;
        }
        switch (enumerationItemAttr) {
            ...
            case 'name':
                if (isCurrentAccessLevelAllowModify) {
                    subMenuIndex = subMenu.push(menuRenameItemMenuItemGenerate(user,`${currentIndex}`, subMenuIndex, dataType, currentItem, 'names', enumerationNamesMain));
                    subMenuIndex = subMenu.push({
                        index: `${currentIndex}.${subMenuIndex}`,
    ...
    if (isCurrentAccessLevelAllowModify){
        switch (dataType) {
            ...
            case dataTypeDeviceButtons:
                const
                    ...
                    currentTranslation = translationsPointOnItemOwner(user, translationType, true),
                    currentIds = currentItem === currentFunction.state ? [currentItem, translationPrimaryStateId] : (enumerationDeviceBasicAttributes.includes(currentItem) ? [] : [currentItem]);
                if (dataType === dataTypeDeviceButtons) {
                    enumerationDeviceButtonsAccessLevelAttrs.forEach(accessLevelsAttr => {
                        const subMenuItem = {
                            index: `${currentIndex}.${subMenuIndex}`,
                        ...
                        let subSubMenuIndex = 0;
                        if (accessLevelsAttr === enumerationAccessLevelToPress) {
                            subSubMenuIndex = subMenuItem.submenu.push({
                                index: `${currentIndex}.${subMenuIndex}.${subSubMenuIndex}`,
                                name: `[${translationsItemTextGet(user, convertButtonToAttribute)}]`,
                                icon: currentEnumerationItem[accessLevelsAttr] === convertButtonToAttribute ? iconItemAttribute : iconItemButton,
                                param: commandsPackParams(cmdItemPress, dataType, currentItem, accessLevelsAttr, dataTypeExtraId, convertButtonToAttribute),
                                submenu: []
                            });
                        }
                        MenuRoles.accessLevels.filter(accessLevel => ! MenuRoles.accessLevelsHidden.includes(accessLevel)).forEach((accessLevel, levelIndex) => {
                ...
                currentIds.forEach(currentId => {
                    ...
                    if ((currentValue === null || currentValue === undefined) || (currentId === currentTranslationId)) {
                        ...
                        currentValue = `${translationsItemGet(user, `${translationCommonFunctionsAttributesPrefix}.${currentId}`)} ${iconItemCommon}`;
                    }
                    else {
                    ...
                    subSubMenuIndex = subMenuItem.submenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeTranslation, dataTypeTranslation, `${currentIndex}.${subMenuIndex}`, subSubMenuIndex, currentTranslationId));
                    subMenuIndex = subMenu.push(subMenuItem);
                });
                break;
        ...
        if (((dataType === dataTypePrimaryEnums) && (enumerationsGetActiveSubItemsCount(dataTypeExtraId, currentItem) === 0)) ||
            ...
            subMenuIndex = subMenu.push(menuDeleteItemMenuItemGenerate(user, dataType, currentItem,`${currentIndex}`, subMenuIndex, dataTypeExtraId));
    }
...
function enumerationsItemMenuItemDetails(user, menuItemToProcess) {
    const
        [cmdId, enumerationType, currentItem, paramToSkip, enumerationTypeExtraId, otherParams] = commandUnpackParams(menuItemToProcess.param),
        currentEnumeration = enumerationGetList(enumerationType, enumerationTypeExtraId),
        currentEnumerationItem = currentEnumeration[currentItem];
    ...
    if (currentEnumerationItem.order > 0) {
        ...
        currentItemDetailsList.push({
            ...
            valueString: previousItem ? enumerationItemName(user, enumerationType, previousItem, currentEnumeration[previousItem]) : ''
        });
    }
    if (currentEnumerationItem.order < (Object.keys(currentEnumeration).length - 1)) {
        ...
        currentItemDetailsList.push({
            ...
            valueString: nextItem ? enumerationItemName(user, enumerationType, nextItem, currentEnumeration[nextItem]) : ''
    });
    }
 ...
 */
function enumerationItemName(user, enumerationType, enumerationItemId, enumerationItem) {
    const currentItemTranslationId = translationsGetEnumId(user, enumerationType, enumerationItemId, enumerationNamesMain);
    // logs(`enumerationType = ${enumerationType}, enumerationItemId = ${enumerationItemId}, currentItemTranslationId = ${currentItemTranslationId}`, l)
    ...
    if (enumerationItem.name && enumerationItemId.includes('.')) {
        ...
        result = `${translationsGetEnumName(user, enumerationType, parentId, enumerationNamesMain)} ${iconItemToSubItemByArrow} ${result}`;
    }
    return result;
}
...
function enumerationsListMenuGenerate(user, menuItemToProcess) {
    ...
    const currentEnumeration = enumerationGetList(enumerationType, enumerationTypeExtraId);
    let
        subMenu = [],
        subMenuIndex = 0;
    Object.keys(currentEnumeration)
        .sort((a, b) => (currentEnumeration[a].order - currentEnumeration[b].order))
        .forEach((currentItem) => {
            // logs(`currentItem = ${JSON.stringify(currentItem)}`, l);
            const
                currentEnumerationItem = currentEnumeration[currentItem],
                currentIcon = currentEnumerationItem.isEnabled ? (currentEnumerationItem.hasOwnProperty('icon') ? currentEnumerationItem.icon : '') : iconItemDisabled;
            // logs(`currentEnumerationItem = ${JSON.stringify(currentEnumerationItem)}`, l);
            const currentMenuItem = {
                    index: `${currentIndex}.${subMenuIndex}`,
                    name: `${enumerationItemName(user, enumerationType, currentItem, currentEnumerationItem)}${currentEnumerationItem.isExternal ? ` ${iconItemIsExternal}`: ''}`,
                    icon: currentIcon,
                    accessLevel: currentAccessLevel,
                    ...
                    submenu: enumerationItemMenuGenerate
            };
            ...
            switch (enumerationType) {
                case dataTypeFunction:
                    subMenuIndex = subMenu.push({
                        ...
                        icon: iconItemCommon,
                        accessLevel: currentAccessLevel,
                        id: `${translationCommonFunctionsAttributesPrefix}`,
 ...
 */
function enumerationIsHistoryEnabledForState(stateObject, historyAdapterId) {
 ...
 * @returns {object[]} Newly generated submenu.
 */
function enumerationMenuGenerateDeviceButtons(user, menuItemToProcess) {
    const
        currentIndex = menuItemToProcess.index !== undefined ? menuItemToProcess.index : '',
        ...
        currentDeviceAttributes.forEach((deviceAttributeId) => {
            ...
            if (existsObject(stateIdFull) && (( stateShortIdSectionsCount == 1) || (isStatesInFolders && ( stateShortIdSectionsCount == 2)))) {
                ...
                if (stateObject && stateObject.hasOwnProperty('common') && stateObject.common) {
                    ...
                    if (enumerationIsHistoryEnabledForState(stateObject, historyAdapterId) && (currentStateType === 'number')) {
                            ...
                            if (currentStateType === 'boolean') {
                                ...
                                if ((states !== undefined) && Object.keys(states).length > 0) {
                                    ...
                                    for (const [possibleValue, possibleName] of Object.entries(states)) {
                                        ...
                                        const subSubMenuItem = {
                                            index: `${currentIndex}.${subMenuIndex}.${subSubMenuIndex}`,
                                            name: `${enumerationStateValueDetails(user, stateObject, currentFunctionId, {val: possibleValue})['valueString'] /*  possibleName !== undefined ? possibleName : possibleValue */}`,
    ...
    if (isCurrentAccessLevelNonSilent) {
        ...
        subMenuIndex = subMenu.push({
            ...
            accessLevel: currentAccessLevel,
            icon: iconItemAlerts,
            function: enumerationDeviceAttributesMenuItemDetails,
            group: 'alerts',
 ...
 * @returns
 */
function enumerationExtractPossibleValueStates(inputStates) {
 ...
 */
function enumerationEvaluateValueConversionCode(user, inputValue, convertValueCode) {
    ...
    if (typeOf(convertValueCode, 'string')) {
        if ((convertValueCode === printDate) || (convertValueCode === `${printDate};`)) {
            try {
                const timeStamp = new Date(inputValue);
                // @ts-ignore
                inputValue = timeStamp.format(configOptions.getOption(cfgDateTimeTemplate, user))
            } catch (error) {
 ...
 */
function enumerationStateValueDetails(user, stateIdOrObject, functionId, currentState) {
    ...
    if (currObject && (typeof(currObject) === 'object') && currObject.hasOwnProperty('_id') && currObject.hasOwnProperty('common')) {
        const
            ...
            currStateVal = enumerationEvaluateValueConversionCode(user, (currentState && currentState.hasOwnProperty('val')) ? currentState.val : (existsState(currentId) ? getState(currentId).val : undefined), convertValueCode),
        ...
        if ( (currObjectType === 'boolean') &&  (currStateValType === 'boolean')) {
            ...
            const states = enumerationExtractPossibleValueStates(currObject.common['states']);
            if (currStateVal !== undefined) {
 ...
 * @returns {string} A formatted string.
*/
function enumerationDeviceAttributesMenuItemDetails(user, menuItemToProcess) {
        ...
        if (functionsList.hasOwnProperty(menuItemToProcess.funcEnum) && currentFunction.hasOwnProperty('deviceAttributes')) {
            ...
            deviceAttributes.forEach((deviceAttribute) => {
                ...
                switch (deviceAttribute) {
                    ...
                    case 'lc':
                        timeStamp = new Date(Number(primaryState.lc));
                        deviceAttributesArray.push({
                            label: translationsGetObjectName(user, deviceAttribute, currentFunctionId, undefined, true),
                            // @ts-ignore
                            valueString: timeStamp.format(configOptions.getOption(cfgDateTimeTemplate, user)),
                            lengthModifier: 0
                        });
                        break;
                    case 'ack':
                    ...
                    default:
                        ...
                        if (existsObject(deviceAttributeId)) {
                            ...
                            deviceAttributesArray.push({label: translationsGetObjectName(user, deviceAttributeId === primaryStateId ? translationPrimaryStateId : currObject, currentFunctionId), ...enumerationStateValueDetails(user, currObject, currentFunctionId, deviceAttributeId === primaryStateId ? primaryState : null)});
                        }
                        break;
                }
            });
 ...
 */
function enumerationGetList(enumerationType, enumerationTypeExtraId) {
 ...
 * @returns
 */
function enumerationRefreshFunctionDeviceStates(functionId, typeOfDeviceStates, isOnlyEnabled) {
    if (enumerationsList[dataTypeFunction].list.hasOwnProperty(functionId) && enumerationsSubTypes.includes(typeOfDeviceStates)) {
        ...
        const
            ...
            destsList = enumerationsList[dataTypeDestination].list,
            destsListKeys = Object.keys(destsList).filter((destId) => (destsList[destId].isEnabled && destsList[destId].isAvailable)).sort((a, b) => (destsList[a].order - destsList[b].order));
        if (typeOfDeviceStates === dataTypeDeviceAttributes) {
            enumerationDeviceBasicAttributes.split(':').forEach((deviceAttr) => {
                ...
                if (! Object.keys(currentDeviceStatesList).includes(deviceAttr)) {
                    currentDeviceStatesList[deviceAttr] = {
                        isEnabled : false,
                        nameTranslationId: translationsGetObjectId(deviceAttr.split('.').join('_'), functionId, undefined, enumerationDeviceBasicAttributes.includes(deviceAttr)),
                        order : Object.keys(currentDeviceStatesList).length
                    }
                }
            });
        }
        // logs(`destsList = ${JSON.stringify(destsList)}`);
        $(`state[id=*${currentFunction.state}](${currentFunction.enum}=${functionId})`).each( (mainId) =>  {
            if (existsObject(mainId)) {
                ...
                if (mainObject.hasOwnProperty('enumIds') ) {
                    for (const destId of destsListKeys) {
                        const fullDestId = `enum.${destsList[destId].enum}.${destId}`;
                        // logs(`idPrefix = ${idPrefix}, fullDestId = ${fullDestId}`, l);
...
function alertsStoreStateValue(alertStateId, currentValue){
    // logs(`alertStateId = ${alertStateId}, currentValue = ${currentValue}`, l)
...
function alertsInit(checkStates) {
    ...
    // logs(`currentlySubscribedStates, length = ${currentlySubscribedStates.length}, list = ${JSON.stringify(currentlySubscribedStates)}`, l);
    statesToSubscribe.forEach(stateId => {
        if (! currentlySubscribedStates.includes(stateId)) {
            // logs(`subscribe on ${stateId}`, l);
...
function alertsMessagePush(user, alertId, alertMessage, isAcknowledged) {
    ...
    const
        ...
        [lastUserMessageId, isUserMessageOldOrNotExists] = cachedGetValueAndCheckItIfOld(user, cachedBotSendMessageId, '95:59:00');
        ...
        alerts[object.id].chatIds.forEach((thresholds, chatId) => {
            ...
            if ((chatId > 0) || (activeChatGroups.includes(chatId))) {
                ...
                const
                    ...
                    alertOldStatus = alertFunctionId ? enumerationStateValueDetails(user, object.id, alertFunctionId, object.oldState)['valueString'] : object.oldState.val,
...
function alertsHistoryMenuGenerate(user, menuItemToProcess) {
    ...
    for (let alertIndex = alertMessages.length - 1; alertIndex >= 0; alertIndex--) {
        const
            ...
            // @ts-ignore
            alertDate = (new Date(alertMessage.date)).format(configOptions.getOption(cfgDateTimeTemplate, user));
        subMenuIndex = subMenu.push({
            index: `${currentIndex}.${subMenuIndex}`,
            ...
            submenu: (user, menuItemToProcess) => {
                ...
                const [subSubMenu,  subSubMenuIndex] = menuNavigationLeftRightMenuPartGenerate(user, [], `${currentIndex}.${subMenuIndex}`, subMenuIndex, currentMessageIndex, alertMessages.length - 1, undefined, true);
                return subSubMenu;
            },
        })
    }
                ...
                if (alertObject && (
                    ...
                    if (
                        ...
                        alertsBy[alertFirstLevelId][alertSecondLevelId][alertTopId][alertId] =
                            translationsGetObjectName(user, alertIdShort === funcsList[alertFuncId].state ? translationPrimaryStateId : alertObject, alertFuncId);
            ...
            if ((! currentFuncId) && (! currentDestId) && (currentObjectIndex === undefined)) {
                ...
                Object.keys(alertsIdList).filter(key => ! ['name', 'index'].includes(key)).sort().forEach(alertId => {
                    alertMenuIndex = subMenu.push({
                        ...
                        submenu: isCurrentAccessLevelAllowModify ? [menuDeleteItemMenuItemGenerate(user, dataTypeAlertSubscribed, alertId, `${currentIndex}.${alertMenuIndex}`, 0)] : []
                    });
                });
            }
        }
        ...
        if (
            ...
            menuItem['submenu'] = (user, menuItemToProcess) => {
                const
                    ...
                    [cmdId, currentStateId, currentFunctionId, currentDestinationId] = commandUnpackParams(menuItemToProcess.param),
                ...
                Object.keys(currentThresholds).sort((thresholdA, thresholdB) => (Number(thresholdA) - Number(thresholdB))).forEach(currentThreshold => {
                    subMenuIndex = subMenu.push({
                        ...
                        submenu: [
                            ...
                            menuDeleteItemMenuItemGenerate(user, dataTypeAlertSubscribed, currentStateId, `${currentIndex}.${subMenuIndex}`, 3, subMenuIndex)
                        ],
                    });
                });
...
function backupMenuGenerate(user, menuItemToProcess) {
    ...
    const
        currentIndex = menuItemToProcess.index !== undefined ? menuItemToProcess.index : '',
        currentId = menuItemToProcess.id,
        currentAccessLevel = menuItemToProcess.accessLevel,
        ...
        if (backupFiles.length) {
            ...
            backupFiles.forEach((fileName, fileIndex) => {
                const
                    ...
                    [fullName, backupDate, backupTime, backupMode] = backupFileDetails && (backupFileDetails.length === 4) ? backupFileDetails : [];
...
function backupRestoreMenuGenerate(user, menuItemToProcess) {
    // logs(`menuItemToProcess = ${JSON.stringify(menuItemToProcess)}`, l);
    const
        currentIndex = menuItemToProcess.index !== undefined ? menuItemToProcess.index : '',
    ...
    subMenuIndex = subMenu.push(menuDeleteItemMenuItemGenerate(user, dataTypeBackup, currentParam, `${currentIndex}`, subMenuIndex));
    return subMenu;
}
...
async function backupCreate(backupMode) {
    ...
    return await backupFileWrite(backupFileName, backupDataJSON, backupMode)
        ...
        .catch(error => {});
    // logs(`Files: ${JSON.stringify(backupGetFolderList(), null, 1)}`, l);
}
...
async function backupRestore(fileName, restoreItem) {
    ...
    if (backupFileMask.test(fileName)) {
        const
            ...
            backupData = await backupFileRead(backupFileName).catch(error => {});
        let restoreData;
        if (backupData) {
            result = true;
            try {
                ...
                itemsToRestore.forEach(itemToRestore => {
                    switch (itemToRestore) {
                        case backupItemConfigOptions:
                            // logs(`1 itemToRestore = ${JSON.stringify(restoreData[itemToRestore])}`, l);
                        ...
                        case backupItemMenuListItems:
                            // logs(`2 itemToRestore = ${JSON.stringify(restoreData[itemToRestore])}`, l);
                            if (restoreData.hasOwnProperty(itemToRestore) && typeOf(restoreData[itemToRestore], 'object')) {
                        ...
                        case backupItemAlerts:
                            // logs(`3 itemToRestore = ${JSON.stringify(restoreData[itemToRestore])}`, l);
                            if (restoreData.hasOwnProperty(itemToRestore) && typeOf(restoreData[itemToRestore], 'object')) {
...
async function backupDeleteOldFiles(mode) {
    if (mode === backupModeAuto) {
        ...
        while (backupFiles.length > maxBackupFiles) {
            await backupFileDelete(backupFiles.shift()).catch(error => {})
        }
    }
    return true
}
            ...
            reportObject.common.members.forEach((member) => {
                if (member) {
                    ...
                    memberId = newMenu.push({
                        ...
                        function: (user, menuItemToProcess) => (`${memberTopObjectName} (${member})`),
                        submenu: [
                            menuDeleteItemMenuItemGenerate(user, dataTypeReportMember, reportId, `${currentIndex}.${memberId}`, 0, memberId)
                        ],
                    })
                }
            })
                ...
                subMenuIndex = subMenu.push({
                    ...
                    submenu: (user, menuItemToProcess) => {
                        ...
                        Object.keys(enumerationsList[dataTypeDestination].list).filter((key) => (enumerationsList[dataTypeDestination].list[key].isEnabled)).forEach((key) => {
                            const isSelected = queryDests.includes(key)? '|' : '';
                            subMenuIndex = subMenu.push({
                                index: `${currentIndex}.${subMenuIndex}`,
                            ...
                            destsList.forEach((destinationId) => {
                                ...
                                $(`${query}${destinationId ? `(${currentDestination.enum}=${destinationId})` : ''}`).each((id, index) => {
                                    ...
                                    subMenu[destIndex].submenu.push({
                                        ...
                                        function: (user, menuItemToProcess) => (`${stateTopObjectName} (${id})`),
                                        submenu: [
                                            {
                                                index: `${currentIndex}.${subMenuIndex-1}.${index}`,
            ...
            if (currentObject.hasOwnProperty('enumIds')) {
                ...
                stateDests.forEach(stateDest => {
                    if (! stateFuncs.length) {
                        stateFuncs.push(enumerationFunctionNotFound)
                    }
...
function simpleReportGenerate(user, menuItemToProcess) {
    ...
    if (menuItemToProcess && menuItemToProcess.hasOwnProperty('id') && Object.keys(reportsList.list).includes(menuItemToProcess.id)) {
        ...
        if (reportObject && reportObject.hasOwnProperty('common') && reportObject.common.hasOwnProperty('members') && Array.isArray(reportObject.common['members']) && reportObject.common['members'].length) {
            ...
            Object.keys(reportStatesStructure).sort((a, b) => (enumerationCompareOrderOfItems(a, b, dataTypeDestination))).forEach(currentDestId => {
                const linePrefix = '- ';
                reportLines.push({label: `${linePrefix}${translationsItemTextGet(user, 'In').toLowerCase()} ${translationsGetEnumName(user, dataTypeDestination, currentDestId, enumerationNamesInside)}`});
                const currentFuncs = Object.keys(reportStatesStructure[currentDestId]);
                currentFuncs.sort((a, b) => (enumerationCompareOrderOfItems(a, b, dataTypeFunction))).forEach(currentFuncId => {
                    if (isAlwaysExpanded || (currentFuncs.length > 1)) {
                        reportLines.push({label: ` ${linePrefix}${stringCapitalize(translationsGetEnumName(user, dataTypeFunction, currentFuncId, enumerationNamesMany))}`});
                    }
                    ...
                    currentDeviceObjects.forEach(currentDeviceObject => {
                        ...
                        currentStateObjects.forEach(currentStateObject => {
                            ...
                            reportLines[reportLines.length - 1] = {...reportLines[reportLines.length - 1], ...enumerationStateValueDetails(user, currentStateObject, currentFuncId)};
                        })
                    })
                })
            });
 ...
 */
 function menuRootMenuItemGenerate(user, topRootMenuItemId) {
    ...
    const
        ...
        currentNameId = isFunctionsFirst ? enumerationNamesMany : enumerationNamesMain,
    ...
    function menuRootSetupMenuItemGenerate(user) {
        ...
        if ((user === null) || (user && user.userId && (MenuRoles.compareAccessLevels(usersInMenu.getMenuItemAccess(user.userId, menuItem.id), rolesAccessLevelForbidden) !== 0))) {
            const subMenu = [
                ...
                {
                    name: translationsItemMenuGet(user, 'Translation'),
                            ...
                            submenu: [
                                {
                                    ...
                                    group: 'menuTranslationFile',
                                    id: doUploadDirectly,
                                    param:  commandsPackParams(cmdItemUpload, dataTypeTranslation, doUploadDirectly),
                                    function: translationsUploadMenuItemDetails,
    ...
    if (menuItem) {
        ...
        if (! isFunctionsFirst) {
            functionsListIds.filter(itemId => (functionsList[itemId].isExternal)).forEach(itemId => {
                const menuItem = menuRootEnumerationMenuItemGenerate(user, dataTypeFunction, functionsList, itemId, enumerationNamesMany);
                if (menuItem) {
                    rootMenu.submenu.push(menuItem);
                }
            });
 ...
 */
function menuDeleteItemMenuItemGenerate(user, dataType, dataItem, upperMenuItemIndex, subMenuItemIndex, dataId, ...dataValues) {
    // logs(`param = ${commandPackParams(cmdItemDeleteConfirm, dataType, dataItem, dataId, ...dataValues)}`, l)
    return {
...
function menuFirstLevelMenuGenerate(user, menuItemToProcess) {
    ...
    if (primaryMenuItemsList.hasOwnProperty(primaryLevelMenuItemId)) {
        const
            ...
            namesCurrent = isFunctionsFirst ? enumerationNamesMain : enumerationNamesMany,
            isFunctionsFirstGlobal = configOptions.getOption(cfgMenuFunctionsFirst),
            inverseMasks = isFunctionsFirst !== isFunctionsFirstGlobal,
            ...
            if (existsObject(stateId)) {
                ...
                if (currentObject.hasOwnProperty('enumIds') ) {
                    secondaryMenuItemsIndex.forEach((currentLevelMenuItemId) => {
                        ...
                        //  logs(`\n shortStateId = ${shortStateId}, currentLevelMenuItemId = ${currentLevelMenuItemId}, currentLevelMenuItem = ${JSON.stringify(currentLevelMenuItem)}`, l);
        ...
        // logs(`deviceList = ${JSON.stringify(deviceList, null, 2)}`, l)
        ...
        Object.keys(deviceList).sort((a, b) => (secondaryMenuItemsList[a].order - secondaryMenuItemsList[b].order)).forEach((currentLevelMenuItemId) => {
            ...
            // logs(`primaryLevelMenuItemId = ${primaryLevelMenuItemId}, currentLevelMenuItemId = ${currentLevelMenuItemId}, currentAccessLevel = ${currentAccessLevel}`, l);
            if (currentAccessLevel && (! MenuRoles.accessLevelsPreventToShow.includes(currentAccessLevel))) {
                ...
                if (MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelForbidden) < 0 ) deviceList[currentLevelMenuItemId].forEach((deviceStateId, deviceIndex) => {
                    const
                    ...
                    const deviceMenuItem = {
                        ...
                        icons: currentIcons,
                        icon: currentIcon,
                        submenu: enumerationMenuGenerateDeviceButtons
                    };
    ...
    // logs('subMenu New = ' + JSON.stringify(subMenu, null, 1), l);
    return subMenu;
}
...
function menuProcessMenuItem(user, cmd, cmdPos, clearBefore, clearUserMessage, isSilent, skipConfirmation) {
     ...
     */
    function executeMenuItem(preparedMessageObject, menuItemToProcess) {
        ...
        if( (cmd) && (menuItemToProcess) && (menuItemToProcess.submenu.length === 0) && ! menuItemToProcess.hasOwnProperty('function')) {
            if ( menuItemToProcess.hasOwnProperty('state') && (menuItemToProcess.state)) {
                ...
                logs('currObject = ' + JSON.stringify(currObject));
                const role = currObject.common['role'];
                    ...
                    if ((currObjectType === 'boolean') || (possibleValue !== undefined)) {
                        if (currObjectType === 'boolean') {
                            setState(currState, ! getState(currState).val, stateSetSuccessfully );
                                ...
                                const currObjectStates = enumerationExtractPossibleValueStates(currObject.common['states']);
                                logs('possibleValue = ' + JSON.stringify(possibleValue));
...
function menuDrawMenuItem(user, itemPos, clearBefore, clearUserMessage, isSilent) {
     ...
     * @param {*} preparedMessageObject -(`object`) The prepared telegram message object (including buttons).
     * @param {*} menuItemToProcess -(`object`) The menu item, which have to be "drawn".
     */
    function drawPreparedMenuItem(preparedMessageObject, menuItemToProcess) {
        ...
        if ((currentSubMenuPos !== undefined) && (currentSubMenuPos >= 0) && (menuItemToProcess.submenu.length > 0) && (currentSubMenuPos < menuItemToProcess.submenu.length)) {
            ...
            for (let buttonsIndex = 0; buttonsIndex < (buttonsCount > maxButtonsCount ? maxButtonsCount : buttonsCount); buttonsIndex++) {
                ...
                const
                    ...
                    currentValue =  currentObject && (currentObject.common['type'] !== 'boolean') ? ` (${enumerationStateValueDetails(user, currentObject, currentSubMenuItem.funcEnum).valueString})`: '',
...
function menuRenewMenuMessage(idOfUser, forceNow) {
    ...
    // logs('userIds = ' + JSON.stringify(userIds), l);
    if (idOfUser === menuRefreshTimeAllUsers) {
        userIds = userIds.concat(telegramGetGroupChats(true));
        // logs(`users = ${JSON.stringify(userIds)}`, l);
    }
    userIds.forEach(userId => {
        // logs('userId = ' + JSON.stringify(userId), l);
        if ((idOfUser == userId) || ((idOfUser === menuRefreshTimeAllUsers) && ((! menuRefreshScheduled.has(userId)) || forceNow))) {
            ...
            // logs('user = ' + JSON.stringify(user), l);
            const
                [lastBotMessageId48, isBotMessageOld48OrNotExists] = cachedGetValueAndCheckItIfOld(user, cachedBotSendMessageId, timeDelta48),
                [lastBotMessageId24, isBotMessageOld24OrNotExists] = cachedGetValueAndCheckItIfOld(user, cachedBotSendMessageId, timeDelta24),
...
function menuSplitButtonsArrayIntoButtonsPerRowsArray(user, buttonsArray) {
    ...
    // logs(`maxTextLength = ${maxTextLength}`, l);
    let
        buttonsRow = [],
        currentGroup = '';
    ...
    // logs('resultArr = ' + JSON.stringify(resultArr));
    return resultArr;
}
...
async function commandUserInputCallback(user, userInputToProcess) {
    ...
    const
        ...
        [currentCommand, currentType, currentItem, currentParam, currentValue, currentSubParam, currentSubValue, ...otherItems] = commandUnpackParams(inputData, [cmdNoOperation]),
        menuMessageObject = {};
    ;
    let
    ...
    if (isWaitForInput) {
        if (userInputToProcess != dataTypeIgnoreInput) {
            switch (currentCommand) {
                case cmdItemUpload:
                    // logs(`currentType = ${currentType}, currentItem = ${currentItem}, currentParam = ${currentParam}, userInputToProcess = ${userInputToProcess}`, l);
                    switch (currentType) {
                    ...
                    switch (currentType) {
                            ...
                            switch (currentParam) {
                                case 'state':
                                    ...
                                    if (currentDeviceAttributes && currentDeviceAttributes.hasOwnProperty(currentParam)) {
                                        ...
                                        currentDeviceAttributes[userInputToProcess].nameTranslationId = translationsGetObjectId(translationsGetObjectId(userInputToProcess.split('.').join('_'), currentItem, undefined, enumerationDeviceBasicAttributes.includes(userInputToProcess)));
                        ...
                        case dataTypeReport:
                            switch (currentParam) {
                                ...
                                case 'names':
                                    enumerationsUpdateItemName(user, currentType, currentItem, enumerationsList[currentType].list[currentItem], userInputToProcess, currentValue !== enumerationNamesMain ? currentValue : '');
                                    break;
                        ...
                        case dataTypeGroups:
                            const currentEnumeration = enumerationGetList(currentParam, currentValue);
                            ...
                            // logs(`currentParam, currentValue = ${[currentType, currentItem, currentParam, currentValue,]}`, l)
                            if (currentValue) {
                                enumerationSave(dataTypeFunction);
                            }
                            else {
        ...
        switch (currentType) {
            ...
            case dataTypeDeviceButtons:
                ...
                if (enumerationsSubTypes.includes(currentType) && enumerationDeviceButtonsAccessLevelAttrs.includes(currentParam)) {
                    ...
                    if ((currentType === dataTypeFunction) && (currentParam === 'isEnabled') && currentList[currentItem][currentParam]) {
                        enumerationRefreshFunctionDeviceStates(currentItem, dataTypeDeviceAttributes, false);
            ...
            case dataTypeGroups:
                const currentEnumeration = enumerationGetList(currentParam, currentSubParam ? currentSubParam : '');
                if (currentItem && currentEnumeration && currentEnumeration.hasOwnProperty(currentItem) && currentEnumeration[currentItem]) {
        ...
        nodeFS.mkdtemp(nodePath.join(nodeOS.tmpdir(), temporaryFolderPrefix), (err, tmpDirectory) => {
            if (err) {
                ...
                const
                    ...
                    currentTranslation = translationsGetCurrentForUser(user);
                nodeFS.writeFile(tmpFileName, JSON.stringify({type: translationType, language: languageId, version: translationVersion, translation: currentTranslation}, null, 2), err => {
                    if (err) {
            ...
            case dataTypeTranslation:
                switch (currentItem) {
                    case doUploadDirectly:
                        ...
                        cachedDelValue(user, cachedTranslationToUpload);
                    ...
                    case doUploadFromRepo:
                        ...
                        translationsLoadLocalesFromRepository(currentLanguageId, (locales, error) => {
            ...
            case dataTypeConfig:
                switch (currentItem) {
                    ...
                    default:
                        ...
                        // logs(`currentItem = ${currentItem}, currentParam = ${currentParam}, configItemArray = ${configItemArray}, currentValue = ${currentValue}`, l)
        ...
        switch (currentType) {
            ...
            case dataTypeDeviceAttributes:
            case dataTypeDeviceButtons:
                if (enumerationRefreshFunctionDeviceStates(currentItem, currentType, false)) menuClearCachedMenuItemsAndRows(user);
                break;

            case dataTypeTranslation:
                if (cachedExistsValue(user, cachedTranslationToUpload)) {
                    const updateTranslationResult = translationsProcessLanguageUpdate(user, currentItem, currentParam);
                    ...
                    case cfgMenuLanguage:
                        if (currentParam === configOptionScopeGlobal) {
                            ...
                            if (! translationsList.hasOwnProperty(newLanguageId)) {
                                ...
                                translationsLoadLocalesFromRepository(newLanguageId, (locales, error) => {
                                    ...
                                    if (locales && typeOf(locales, 'object') && locales.hasOwnProperty(newLanguageId) && typeOf(locales[newLanguageId], 'object')) {
                                        if (translationsCheckAndCacheUploadedFile(user, '', '', '', locales[newLanguageId])) {
                                            ...
                                            if (newTranslation && newTranslation.hasOwnProperty('translation') && typeOf(newTranslation['translation'], 'object') && (Math.abs(translationCompareVersion(newTranslation['version'])) < 10)) {
                                                translationsList[newLanguageId] = newTranslation['translation'];
                                            }
                                        }
                                    }
                                    translationsSave()
                ...
                if (existsObject(graphTemplateId) && ((currentParam === dataTypeReport) || existsState(currentItem))) {
                    ...
                    if (graphTemplate.native && graphTemplate.native.data && graphTemplate.native.data.lines && (graphTemplate.native.data.lines.length === 1)) {
                        if (currentParam === dataTypeReport) {
                                    ...
                                    currentFuncs.sort((a, b) => (enumerationCompareOrderOfItems(a, b, dataTypeFunction))).forEach(currentFuncId => {
                                        ...
                                        currentDeviceObjects.forEach(currentDeviceObject => {
                                            ...
                                            currentStateObjects.forEach(currentStateObject => {
                                                ...
                                                currentLine.name = `${translationsItemTextGet(user, 'In').toLowerCase()} ${translationsGetEnumName(user, dataTypeDestination, currentDestId, enumerationNamesInside)}`;
                            ...
                            graphTemplate.native.data.title = `${translationsGetEnumName(user, dataTypeFunction, currentSubParam, enumerationNamesMain)} ${translationsItemTextGet(user, 'In').toLowerCase()} ${translationsGetEnumName(user, dataTypeDestination, currentSubValue, enumerationNamesInside)}`;
                        }
        ...
        switch (currentType) {
            case dataTypeConfig:
                if (! configGlobalOptions.includes(currentItem)) {
                    ...
                    // logs(`exists = ${JSON.stringify(configOptions.existsOption(currentItem, user))}, currentItem = ${JSON.stringify(configOptions.getOption(currentItem, user))}`, l)
                    menuClearCachedMenuItemsAndRows(user);
                }
                break;
        ...
        const
            commonTranslationId = `${translationCommonFunctionsAttributesPrefix}.${currentItem.split('.').pop()}`,
        ...
        if ((! isNaN(chatId)) && (chatId < 0)) {
            if (activeOnly) {
                const
                    ...
                    [lastBotMessageId, isBotMessageOldOrNotExists] = cachedGetValueAndCheckItIfOld(user, cachedBotSendMessageId, timeDelta48),
                    [lastUserMessageId, isUserMessageOldOrNotExists] = cachedGetValueAndCheckItIfOld(user, cachedLastMessage, timeDelta96);
...
function telegramMessagesFormatAndPushToQueueMessage(user, preparedMessageObject, clearBefore, clearUserMessage, createNewMessage, isSilent) {
    ...
    if (alertMessages.length && (preparedMessageObject.buttons !== undefined) ) {
        // @ts-ignore
        const alertDate = (new Date(alertMessages[alertMessages.length - 1].date)).format(configOptions.getOption(cfgDateTimeTemplate, user));
    ...
    if (isMenuOn || createNewMessage) {
        // @ts-ignore
        const timeStamp = '<i>' + (new Date()).format(configOptions.getOption(cfgDateTimeTemplate, user)) + '</i> ';
        ...
        if ((lastMessage != JSON.stringify(preparedMessageObject)) || createNewMessage || clearBefore) {
            ...
            // logs(`telegramObjects = ${JSON.stringify(telegramObjects, null, 2)}`, l)
    ...
    function telegramSendToCallBack(result, user, telegramObject, telegramObjects, currentLength, sendToTS, waitForLog) {
        ...
        if (! result) {
            if (waitForLog) {
                ...
                // logs(`telegramLastErrors = ${JSON.stringify(telegramLastErrors)}, ${telegramLastErrors.hasOwnProperty(`${user.chatId}`)}`, l)
    ...
    if (typeOf(logRecord, 'object') && logRecord.hasOwnProperty('from') && (logRecord.from === telegramAdapter)) {
        try {
            ...
            if (telegramErrorParsed && (telegramErrorParsed.length === 6) &&
                ...
                // logs(`telegramLastErrors[${telegramErrorParsed[2]}] = ${JSON.stringify(telegramLastErrors[telegramErrorParsed[2]])}`, l)
            }
        } catch (error) {
        }
    }
                        ...
                        if (chatId != undefined) {
                            ...
                            if (command !== undefined) {
                                ...
                                if (cachedExistsValue(user, cachedIsWaitForInput)) {
                                    const [isWaitForInput, currentType, currentItem, currentParam, currentValue, currentSubParam, currentSubValue, ...otherItems] = commandUnpackParams(cachedGetValue(user, cachedIsWaitForInput));
...
async function autoTelegramMenuInstanceInit() {
    ...
    if (configOptions.getOption(cfgConfigBackupCopiesCount)) {
        backupScheduleReference = schedule({hour: 2, minute: 5},() => {backupCreate(backupModeAuto)});
    }
...
// @ts-ignore
await autoTelegramMenuInstanceInit();
...
function objectAssignToTemplateLevelOne(template, inputObject, level) {
    ...
    throw new Error("Unable to assign obj to template! Its type isn't supported.");
}
...
function logs(txt, debug) {
    if ((configOptions.getOption(cfgDebugMode)) || (debug !== undefined)){
        log((arguments.callee.caller  ? arguments.callee.caller.name : '') + ': ' + txt);
    }
}

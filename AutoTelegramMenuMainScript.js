...
class ConfigOptions {
  ...
  menuGenerateForArray(user, menuItemToProcess) {
    ...
    if (isThisLevelAllowModify) {
      subMenu.push(menuMenuItemGenerateAddItem(user, currentIndex, subMenuIndex, dataTypeConfig, cfgItem, optionScope, subMenuIndex));
    }
    return subMenu;
  }
                ...
                Object.keys(translationsList).sort().forEach((languageId) => {
                  ...
                  if (currentLanguage !== languageId) {
                    ...
                    subSubMenuIndex = subMenuItem.submenu.push({
                      ...
                      options: {dataType: dataTypeConfig, item: cfgItem, scope: optionScope, value: languageId},
                      submenu: []
                    });
                          ...
                          subMenu.push(
                            {
                              ...
                              command: commandsParamsPack(cmdItemsProcess, dataTypeConfig, cfgItem, optionScope, newLanguageId),
                          ...
                          subMenu.push(
                            {
                              ...
                              icon: iconItemEdit,
                              command: commandsParamsPack(cmdGetInput, dataTypeConfig, cfgItem, optionScope),
              ...
              subMenuItem.submenu = (user, menuItemToProcess) => {
                ...
                currentIntervals.forEach(({id: graphsIntervalId, minutes: graphsIntervalMinutes}, graphsIntervalIndex) => {
                  ...
                  if (isThisLevelAllowModify) {
                    ...
                    subMenuItem.submenu.push(menuMenuItemGenerateDeleteItem(user, `${currentIndex}.${subMenuIndex}`, subSubMenuIndex, dataTypeConfig, cfgItem, optionScope, graphsIntervalIndex));
                  }
                  subMenuIndex = subMenu.push(subMenuItem);
                });
                if (isThisLevelAllowModify) {
            ...
            default: {
              ...
              switch (itemType) {
                ...
                case 'number': {
                  ...
                  subMenuItem.command = commandsParamsPack(cmdGetInput, dataTypeConfig, cfgItem, optionScope);
                  break;
                }
                case 'string': {
...
class MenuRoles {
  ...
  #menuRuleSetAccessLevel(user, menuItemToProcess) {
    ...
    if ((isForNewRule || cachedValueExists(user, cachedRolesNewRule)) && MenuRoles.accessLevels.includes(currentRule.accessLevel)) {
      subMenuIndex = subMenu.push({
        ...
        icon: iconItemApply,
        group: cmdItemsProcess,
        command: commandsParamsPack(cmdItemsProcess, dataType, roleId),
  ...
  #menuGenerateRoleRulesAddItem(user, menuItemToProcess) {
    ...
    subMenuIndex = subMenu.push({
      ...
      name: `${translationsItemMenuGet(user, 'SetAccelLevel')}[${savedRule.mask}]`,
      icon: iconItemEdit,
      options: {dataType: dataTypeMenuRoleRules, roleId, ruleIndex: -1},
    ...
    currentRoleRules.forEach(rule => {
      ...
      if ((roleId === rolesDefaultAdmin) || (MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) >= 0)) {
        ...
        subSubId = currentItem.submenu.push({
          ...
          // icon: iconItemDelete,
          options: {dataType: dataTypeMenuRoleRules, roleId, ruleIndex: subMenuIndex},
          text: ruleCurrentDetails,
          submenu: (user, menuItemToProcess) => {return this.#menuRuleSetAccessLevel(user, menuItemToProcess)}
        });
    ...
    if ((roleId !== rolesDefaultAdmin) &&  (isNewRole || (MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) < 0))) {
      ...
      if ((isNewRole && currentRoleRules.length) || (JSON.stringify(this.getRules(roleId)) !== JSON.stringify(currentRoleRules))) {
        subMenuIndex = subMenu.push({
          ...
          icon: iconItemApply,
          group: cmdItemsProcess,
          command: commandsParamsPack(cmdItemsProcess, dataTypeMenuRoles, roleId),
          options: {dataType: dataTypeMenuRoleRules, roleId},
          submenu: []
        });
      }
      else if ((! cachedValueExists(user, cachedRolesRoleUnderEdit)) && ((this.existsId(roleId)) && (rolesInMenu.getUsers(roleId).length === 0))) {
        subMenu.push(menuMenuItemGenerateDeleteItem(user, `${currentIndex}`, subMenuIndex, dataTypeMenuRoles, roleId));
      }
    }
    return subMenu;
  }
              ...
              subMenu.push(
                {
                  ...
                  icon: iconItemEdit,
                  command: commandsParamsPack(cmdGetInput, dataTypeMenuRoles, '', 'fixId'),
                  options: {dataType: dataTypeMenuRoles, editMode: 'fixId'},
                  submenu: [],
                }
              );
            }
            ...
            subMenu.push(
              {
                ...
                name: `${translationsItemCoreGet(user, 'cmdSetId')}`,
                icon: iconItemEdit,
                command: commandsParamsPack(cmdGetInput, dataTypeMenuRoles, '', 'setId'),
                options: {dataType: dataTypeMenuRoles, editMode: 'setId'},
                submenu: [],
              }
            );
          }
...
function alertsMenuGenerateSubscribed(user, menuItemToProcess) {
  ...
  const
    ...
    currentAccessLevel = menuItemToProcess.accessLevel,
    isCurrentAccessLevelAllowModify = MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) < 0,
    {item: currentObjectIndex} = menuItemToProcess.options;
  let
    alertsBy = {},
    subMenu = [];
      ...
      if ((! currentFuncId) && (! currentDestId) && (currentObjectIndex === undefined)) {
        ...
        Object.keys(objectsIdList).sort().forEach(objectId => {
          objectMenuIndex = subMenu.push({
            ...
            accessLevel: currentAccessLevel,
            options: {item: objectsIdList[objectId]['index']},
            submenu: alertsMenuGenerateSubscribed
          });
        });
      }
  ...
  alertPropagateDistributions.forEach(alertPropagateDistribution => {
    ...
    alertPropagateOptions.forEach(alertPropagateOption => {
      subSubSubMenuItemIndex = subSubMenuItem.submenu.push({
        ...
        icon: '',
        command: commandsParamsPack(cmdItemsProcess, dataTypeAlertSubscribed, alertPropagateDistribution, alertPropagateOption, currentStateId, currentFunctionId, currentDestinationId),
        options: {dataType: alertPropagateDistribution, mode: alertPropagateDistribution, function: currentFunctionId, destination: currentDestinationId, state: currentStateId},
        submenu: []
      });
    });
...
function backupMenuGenerateBackupAndRestore(user, menuItemToProcess) {
  ...
  if (! MenuRoles.accessLevelsPreventToShow.includes(currentAccessLevel)) {
    subMenuIndex = subMenu.push({
        ...
        id: backupModeCreate,
        command: MenuRoles.compareAccessLevels(currentAccessLevel, rolesAccessLevelReadOnly) < 0 ? commandsParamsPack(cmdItemsProcess, dataTypeBackup, backupModeCreate) : cmdNoOperation,
...
function backupMenuGenerateRestore(user, menuItemToProcess) {
  ...
  Object.keys(backupRestoreItemsList).forEach(backupItem => {
    subMenuIndex = subMenu.push({
      ...
      icon: iconItemBackupRestore,
      command: commandsParamsPack(cmdItemsProcess, dataTypeBackup, backupModeRestore, fileName, backupItem),
      options: {dataType: dataTypeBackup, mode: backupModeRestore, fileName, item: backupItem},
      submenu: []
    });
  });
  subMenuIndex = subMenu.push(menuMenuItemGenerateDeleteItem(user, `${currentIndex}`, subMenuIndex, dataTypeBackup, fileName));
  return subMenu;
}
    ...
    memberId = newMenu.push({
      ...
      submenu: (user, menuItemToProcess) => {
        ...
        subMenuIndex = subMenu.push({
          ...
          icon: iconItemEdit,
          command: commandsParamsPack(cmdGetInput, dataTypeReportMember, 'queryState'),
        ...
        subMenuIndex = subMenu.push({
          ...
          icon: iconItemEdit,
          command: commandsParamsPack(cmdGetInput, dataTypeReportMember, 'queryRole'),
          ...
          subMenu.push({
            ...
            submenu: (user, menuItemToProcess) => {
              ...
              subMenuIndex = subMenu.push({
                ...
                group: cmdItemsProcess,
                command: commandsParamsPack(cmdItemsProcess, dataTypeReportMember, reportId, 'marked'),
              ...
              subMenuIndex = subMenu.push({
                ...
                icon: enumerationsList[dataTypeReport].icon,
                command: commandsParamsPack(cmdItemsProcess, dataTypeReportMember, reportId, doAll),
        ...
        newMenu.push(
          {
            ...
            name: `${translationsItemCoreGet(user, 'cmdFixId')}`,
            icon: iconItemEdit,
            command: commandsParamsPack(cmdGetInput, dataTypeReport, '', 'fixId'),
      ...
      newMenu.push(
        {
          ...
          name: `${translationsItemCoreGet(user, 'cmdSetId')}`,
          icon: iconItemEdit,
          command: commandsParamsPack(cmdGetInput, dataTypeReport, '', 'setId'),
        ...
        if (graphsIntervals && typeOf(graphsIntervals, 'array') && graphsIntervals.length) {
          graphsIntervals.forEach(({id: graphsIntervalId, minutes: graphsIntervalMinutes}) => {
            ...
            subMenuIndex = subMenu.push({
              ...
              icon: iconItemChart,
              group: cmdItemsProcess,
              command: commandsParamsPack(cmdItemsProcess, dataTypeGraph, graphsTemplate, dataTypeReport, graphsIntervalMinutes, reportId),
...
function menuMenuItemGenerateDeleteItem(user, upperMenuItemIndex, subMenuItemIndex, dataType, dataItem, dataId, ...dataValues) {
  ...
  return {
    ...
    icon: iconItemDelete,
    group: cmdItemDelete,
    command: commandsParamsPack(cmdItemDelete, dataType, dataItem, subMenuItemIndex, dataId, ...dataValues),
    submenu: [
      {
        ...
        command: commandsParamsPack(cmdItemDeleteConfirm, dataType, dataItem, dataId, ...dataValues),
        submenu: [],
      }
    ],
  };
...
function menuMenuItemGenerateEditItem(_user, upperMenuItemIndex, subMenuItemIndex, itemName, itemGroup, ...commandParams) {
  const result = {
    ...
    name: itemName,
    icon: iconItemEdit,
    command: commandsParamsPack(cmdGetInput, ...commandParams),
    submenu: [],
  };
...
function menuMenuItemGenerateResetItem(user, upperMenuItemIndex, subMenuItemIndex, ...commandParams) {
  return {
    ...
    icon: iconItemReset,
    command: commandsParamsPack(cmdItemReset, ...commandParams),
    group: 'reset',
    submenu: []
  };
}
...
function menuMenuPartGenerateMoveItemUpAndDown(user, subMenu, upperMenuItemIndex, subMenuItemIndex, currentItemIndex, lastItemIndex, groupId, ...commandParams) {
  if (currentItemIndex > 0) {
    subMenuItemIndex = subMenu.push({
      index: `${upperMenuItemIndex}.${subMenuItemIndex}`,
      name: `${iconItemMoveUp}`,
      command: commandsParamsPack(cmdItemMoveUp, ...commandParams),
  ...
  if (currentItemIndex < lastItemIndex) {
    subMenuItemIndex = subMenu.push({
      ...
      command: commandsParamsPack(cmdItemMoveDown, ...commandParams),
      group: groupId === undefined ? 'moveItem' : groupId,
      submenu: [],
    });
  }
...
function menuMenuObjectPrepareOnPosition(user, menuItemToProcess, targetMenuPos, preparedMessageObject, currentIndent, callback) {
  ...
  if (menuItemToProcess.externalMenu) {
    ...
    if ((currentSubMenuPos !== undefined) && (currentSubMenuPos >= 0) && (menuItemToProcess.submenu.length > 0) && (currentSubMenuPos < menuItemToProcess.submenu.length)) {
      ...
      if ((currentSubMenuMaxIndex > 0) && ((preparedMessageObject.options && preparedMessageObject.options[menuOptionHorizontalNavigation]) || configOptions.getOption(cfgShowHorizontalNavigation, user)) &&
      (! (preparedMessageObject.options && (preparedMessageObject.options[menuOptionHorizontalNavigation] === false)))) {
      ...
      for (let buttonsIndex = 0; buttonsIndex < (buttonsCount > maxButtonsCount ? maxButtonsCount : buttonsCount); buttonsIndex++) {
        ...
        let
          callbackData = menuItemButtonPrefix + currentSubIndex,
          currentOptions;
        if (currentSubMenuItem.hasOwnProperty('command') && (typeof(currentSubMenuItem.command) === 'string') && (currentSubMenuItem.command.indexOf(cmdPrefix) === 0) && (currentSubMenuItem.command !== cmdPrefix)) {
            ...
            if (currentSubMenuItem.options) {
              currentOptions = objectDeepClone(currentSubMenuItem.options);
              if (currentOptions.hasOwnProperty(menuOptionHorizontalNavigation)) delete currentOptions[menuOptionHorizontalNavigation];
              callbackData = commandsParamsPack(commandsParamsUnpack(currentSubMenuItem.command).shift(), JSON.stringify({options: currentOptions}));
            }
            /** */
        }
        ...
        if (callbackData.length > 64) {
           ...
           */
          if (currentOptions) {
            callbackDataToCache.set(currentSubIndex, currentOptions);
            callbackData = commandsParamsPack(commandsParamsUnpack(callbackData).shift(), JSON.stringify({options: {cachedId: currentSubIndex}}));
          }
          else {
 ...
 */
function menuMenuReIndex(inputMenu, indexPrefix) {
  let newMenuRow;
 ...
 */
function commandsExtractCommandWithOptions(user, userInputString) {
  let currentOptions = {};
  const [currentCommand, inputOptionsString] = commandsParamsUnpack(userInputString, [cmdNoOperation]);
  if (inputOptionsString) {
    try {
      let inputOptions = JSON.parse(inputOptionsString);
      if (inputOptions.hasOwnProperty('options')) {
        if (inputOptions.options.hasOwnProperty('cachedId')) {
          const
            longCommandId = inputOptions.options['cachedId'],
            cachedLongCommands = cachedValueExists(user, cachedMenuLongCommandsWithParams) ? cachedValueGet(user, cachedMenuLongCommandsWithParams) : {};
          if (cachedLongCommands.has(longCommandId)) {
            currentOptions = cachedLongCommands.get(longCommandId);
          }
        }
        else {
          currentOptions = inputOptions.options;
        }
      }
    } catch (error) {
      console.warn(`Can't parse options for the command '${currentCommand}' from string = '${inputOptionsString}! Error is ${JSON.stringify(error)}'.`);
    }
  }
  return {command: currentCommand, options: currentOptions};
}
...
async function commandsUserInputProcess(user, userInputToProcess) {
  ...
  let
    ...
    currentCommand, currentType, currentItem, currentParam, currentValue, currentSubParam, currentSubValue, _otherItems, commandOptions;
  if (userInput.includes('{"options":{')) {
    [currentCommand, commandOptions] = commandsExtractCommandWithOptions(user, userInput);
  }
  else {
    const
      isLongCommandsCached = userInput.indexOf(cmdCached) === 0,
      longCommandId = isLongCommandsCached ? commandsParamsUnpack(userInput).pop() : null,
    ...
    [currentCommand, currentType, currentItem, currentParam, currentValue, currentSubParam, currentSubValue, ..._otherItems] = commandsParamsUnpack(inputData, [cmdNoOperation]);
  }
        ...
        default: {
          switch (currentType) {
            case dataTypeTranslation: {
              ...
              if (currentItem && currentParam && (! currentValue)) {
            ...
            case dataTypePrimaryEnums: {
              if (currentValue && enumerationsList[currentValue]) {
                ...
                if (currentItem && currentEnumsList[currentItem]) {
                  const
            ...
            case dataTypeDeviceAttributes:
            case dataTypeDeviceButtons: {
              if (currentItem && currentValue && enumerationsList[dataTypeFunction].list[currentValue]) {
                const currentListItem = enumerationsList[dataTypeFunction].list[currentValue][currentType][currentItem];
                if (currentListItem) {
                  if (currentParam && ((currentParam !== 'convertValueCode') || enumerationsTestValueConversionCode(user, currentValue, currentItem, userInputToProcess))) {
    ...
    switch (currentCommand) {
      ...
      case cmdItemPress: {
        switch (commandOptions.dataType) {
          ...
          case dataTypeDeviceButtons: {
            ...
            if (currentParam !== undefined) {
              if (enumerationsSubTypes.includes(commandOptions.dataType) && enumerationsDeviceButtonsAccessLevelAttrs.includes(commandOptions.attribute)) {
                ...
                if ((currentType === dataTypeFunction) && (currentParam === 'isEnabled') && currentList[commandOptions.item][commandOptions.attribute]) {
          ...
          case dataTypeGroups: {
            const currentEnumeration = enumerationsGetList(commandOptions.dataType, commandOptions.dataTypeExtraId);
            if (commandOptions.item && currentEnumeration && currentEnumeration.hasOwnProperty(commandOptions.item) && currentEnumeration[commandOptions.item]) {
              currentEnumeration[commandOptions.item].group = currentEnumeration[commandOptions.item].group === currentValue ? '' : currentValue;
            }
            if (commandOptions.dataTypeExtraId) {
              enumerationsSave(commandOptions.dataTypeExtraId);
            }
            else {
              enumerationsSave(commandOptions.dataType);
            }
            break;
          }
      ...
      case cmdItemsProcess: {
        switch (currentType) {
          ...
          case dataTypeDeviceAttributes:
          case dataTypeDeviceButtons: {
            if (enumerationsRefreshFunctionDeviceStates(user, currentItem, currentType, false)) menuMenuItemsAndRowsClearCached(user);
            break;
          }
                    ...
                    if (currentStateObject) {
                      ...
                      if (toProcessState) {
                        ...
                        if (isDifferentAlertDetails) {
                          if ((currentStateAlertDetails && (currentParam === 'alertPropagateOverwrite')) || (currentStateAlertDetails === undefined)) {
            ...
            if (chatId != undefined) {
              ...
              if (command !== undefined) {
                ...
                if (cachedValueExists(user, cachedIsWaitForInput)) {
                  const [isWaitForInput, currentType, currentItem, _currentParam, _currentValue, _currentSubParam, _currentSubValue, ..._otherItems] = commandsParamsUnpack(cachedValueGet(user, cachedIsWaitForInput));
                  const [currentCommand, commandOptions] = commandsExtractCommandWithOptions(user, cachedValueGet(user, cachedIsWaitForInput));
                  // logs(`isWaitForInput = ${isWaitForInput}, subMenuItemId = ${subMenuItemId}`);
                  if (currentCommand === cmdItemUpload) {
                    cachedValueSet(user, cachedIsWaitForInput, commandsParamsPack(cmdItemUpload, JSON.stringify({...commandOptions, fileName: request.document.file_name, fileSize: request.document.file_size})));

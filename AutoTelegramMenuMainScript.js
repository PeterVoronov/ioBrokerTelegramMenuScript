...
const cmdPrefix = 'cmd',
  ...
  dataTypeAssociatedExtensions = 'associatedExtensions',
...
class ConfigOptions {
  ...
  getMaskDescription(cfgItem) {
    if (this.configOptions.hasOwnProperty(cfgItem)) {
      if (typeof this.configOptions[cfgItem]?.template?.text === 'string') {
  ...
  parseOption(cfgItem, value) {
    ...
    let result,
      valueToParseType = typeToText(value);
    if (cfgItemType === 'array') {
      ...
      if (currentMask instanceof RegExp) {
        ...
        if (cfgItemPath.length === 2) {
          ...
          if (
            ...
            if (this.externalSubscriptions.has(cfgItem)) {
              ...
              if (typeof functionToProcess === 'function') {
  ...
  saveOption(cfgItem, user) {
    let currentValue = this.getOption(cfgItem, user),
      stateType = typeToText(currentValue);
  ...
  restoreDataFromBackup(backupData) {
    if (
      ...
      !Array.isArray(backupData.globalConfig)
    ) {
      ...
      if (this.perUserConfig.size) {
        this.perUserConfig.forEach((userConfig, userId) => {
          const userConfigBackup =
            ...
            backupData.perUserConfig instanceof Map &&
          ...
          optionScopes.forEach((optionScope) => {
            ...
            if (isThisLevelAllowModify) {
              if (isDefined(configOptionsParameters[cfgItem])) {
                ...
                if (typeof interimItem === 'object' && !Array.isArray(interimItem)) {
...
class MenuRoles {
  ...
  initRules(itemId) {
    if (isDefined(itemId)) {
      ...
      if (Array.isArray(this.data[itemId])) this.data[itemId] = MenuRoles.sortRules(this.data[itemId]);
  ...
  load() {
    ...
    if (existsState(telegramUsersId)) {
      ...
      if (
        ...
        !Array.isArray(telegramUsersListState)
      ) {
...
function translationsProcessLanguageUpdate(user, translationInfo, translationUpdateMode) {
          ...
          if (translationCurrentPart.hasOwnProperty(translationKey)) {
            if (
              ...
              if (
                ...
                typeof translationCurrentPart[translationKey] === 'string'
              ) {
        ...
        Object.keys(translationTemplatePart).forEach((translationKey) => {
          if (translationCurrentPart.hasOwnProperty(translationKey)) {
            if (
              ...
              typeof translationTemplatePart[translationKey] === 'string' &&
              typeof translationCurrentPart[translationKey] === 'string'
            ) {
  ...
  if (cachedValueExists(user, cachedTranslationsToUpload)) {
    ...
    if (
      ...
      !Array.isArray(translationInputFull[idTranslation]) &&
...
function translationsGetObjectId(object, functionId, destinationId, isCommon = false) {
  ...
  if ((functionId && typeof functionId === 'string' && functionsList.hasOwnProperty(functionId)) || isCommon) {
    if (
      ...
      if (typeof object?._id === 'string' || typeof object === 'string') {
...
function translationsGetObjectName(user, object, functionId, destinationId, isCommon = false) {
  ...
  if (!result) {
    ...
    if (object?.common?.hasOwnProperty('name')) {
      ...
      } else if (typeof objectCommonName === 'object' && !Array.isArray(objectCommonName)) {
...
function extensionsActionOnRegisterToAutoTelegramMenu(extensionDetails, callback) {
  ...
  switch (type) {
    ...
    default: {
      if (
        typeof extensionsList?.[extensionId] !== 'object' ||
        ...
        warns(
          `Extension "${extensionId}" is already registered with different type or script!` +
            ' Will not update already registered!',
        );
      }
      break;
    }
...
const enumerationsNamesMain = 'Main',
  ...
  enumerationDeviceSubTypes = [
    ...enumerationsDeviceStatesTypes,
    dataTypeDeviceStatesAttributes,
    dataTypeAssociatedExtensions,
  ],
...
function enumerationsLoad(enumerationType) {
  ...
  if (existsState(enumerationsList[enumerationType].state)) {
    try {
      ...
      if (typeof parsedObject?.list === 'object' && parsedObject.list) {
        enumerationsList[enumerationType].list = parsedObject.list;
      }
      if (typeof parsedObject?.enums === 'object' && parsedObject.enums) {
        enumerationsList[enumerationType].enums = parsedObject.enums;
      }
    } catch (error) {
      warns(`Can't load enumeration of type "${enumerationType}". Error is "${jsonStringify(error)}"`);
    }
  }
}
...
function enumerationsReorderItems(currentEnumerations) {
  let countItems = 0;
  if (typeof currentEnumerations === 'object' && !Array.isArray(currentEnumerations)) {
    ...
    for (const currentEnum of getEnums(enumType)) {
      ...
      if (isCurrentItemAcceptable) {
        if (
          ...
          !Array.isArray(currentEnumerationList?.[currentItem])
        ) {
...
function enumerationsUpdateItemName(
  ...
  if (newName && typeof newName === 'string') {
    ...
    if (enumerationItem.hasOwnProperty('names') && Array.isArray(enumerationItem.names)) {
...
function enumerationsMenuGenerateEnumerationItem(user, menuItemToProcess) {
  ...
  const currentEnumerationItem = currentEnumerationsList[itemIdCurrent],
    currentEnumerationItemEnum = currentEnumerationItem.enum,
    lastItemIndex = Object.keys(currentEnumerationsList).length - 1,
    primaryEnumType = enumerationsGetPrimaryDataType(enumerationType, enumerationTypeExtraId),
    primaryEnumsList = enumerationsGetList(primaryEnumType),
    primaryEnumItem = primaryEnumsList[enumerationTypeExtraId];
  ...
  if (currentEnumerationItem?.isEnabled) {
    ...
    if (enumerationType === dataTypeAssociatedExtensions) {
      if (!extensionsList?.[itemIdCurrent]?.isAvailable) {
        currentIcon = iconItemNotFound;
      } else if (currentEnumerationItem?.isEnabled) {
        currentIcon = extensionsList?.[itemIdCurrent]?.icon || '';
      }
    }
  }
  if (isCurrentAccessLevelAllowModify) {
    ...
    if (
      ...
      [subMenu, subMenuIndex] = menuMenuPartGenerateMoveItemUpAndDown(
        ...
        lastItemIndex,
        {dataType: enumerationType, item: itemIdCurrent, dataTypeExtraId: enumerationTypeExtraId},
        'UpDown',
      );
    }
  }
  ...
  switch (enumerationType) {
    case dataTypeFunction:
      if (!currentEnumerationItem.isExternal) {
        ...
        if (!enumerationItemAttrs.includes('associatedExtensions')) {
          enumerationItemAttrs.splice(enumerationItemAttrs.indexOf('devicesTranslation'), 0, 'associatedExtensions');
        }
  ...
  for (let enumerationItemAttr of enumerationItemAttrs) {
    switch (enumerationItemAttr) {
      ...
      case 'extensionId':
      case 'extensionAttributeId': {
        break;
      }

      case 'associatedExtensions':
        ...
        switch (enumerationItemAttr) {
          case 'associatedExtensions': {
            devicesMenuItem.submenu.push({
              ...
              accessLevel: currentAccessLevel,
              options: {dataType: enumerationItemAttr, dataTypeExtraId: itemIdCurrent},
        ...
        if (currentEnumerationItem.isExternal !== true) {
          subMenuIndex = subMenu.push({
            ...
            options: {
              ...
              dataTypeExtraId: {dataType: enumerationType, function: enumerationTypeExtraId, item: itemIdCurrent},
            },
            icon: '',
            submenu: enumerationsMenuGenerateListOfEnumerationItems,
          });
      ...
      case 'targetStateId': {
        if (
          ...
          const valueCurrent = currentEnumerationItem[enumerationItemAttr],
            associatedExtensions = primaryEnumItem?.associatedExtensions || {},
            associatedExtensionsAvailableKeys = Object.keys(associatedExtensions).filter(
              (extensionId) =>
                associatedExtensions[extensionId].isEnabled &&
                extensionsList[extensionId]?.type === extensionTypeAttributes &&
                extensionsList[extensionId]?.isAvailable,
            ),
            associatedExtensionsAvailableCount = associatedExtensionsAvailableKeys.length;
          if (
            associatedExtensionsAvailableCount > 0 &&
            associatedExtensionsAvailableKeys.includes(currentEnumerationItem?.extensionId)
          ) {
            const availableStatesMap = new Map();
            Object.keys(primaryEnumItem.deviceAttributes).forEach((deviceAttributeId) => {
              const deviceAttribute = primaryEnumItem.deviceAttributes[deviceAttributeId];
              if (!deviceAttribute.isExternal) {
                availableStatesMap.set(
                  deviceAttributeId,
                  enumerationsItemName(user, dataTypeDeviceAttributes, deviceAttributeId, deviceAttribute),
                );
              }
            });
            Object.keys(primaryEnumItem.deviceButtons).forEach((deviceAttributeId) => {
              const deviceAttribute = primaryEnumItem.deviceButtons[deviceAttributeId];
              if (!deviceAttribute.isExternal && !availableStatesMap.has(deviceAttributeId)) {
                availableStatesMap.set(
                  deviceAttributeId,
                  enumerationsItemName(user, dataTypeDeviceButtons, deviceAttributeId, deviceAttribute),
                );
              }
            });
            const valueCurrentText = availableStatesMap.has(valueCurrent)
              ? availableStatesMap.get(valueCurrent)
              : valueCurrent;
            subMenuIndex = subMenu.push(
              menuMenuItemGenerateSelectItem(
                ...
                currentIndex,
                subMenuIndex,
                `${translationsItemMenuGet(user, enumerationItemAttr)} [${valueCurrentText}]`,
                availableStatesMap,
                'targetStateId',
                {
                  dataType: enumerationType,
      ...
      case 'name': {
        if (isCurrentAccessLevelAllowModify) {
          if (!enumerationsDeviceStatesTypes.includes(enumerationType) || currentEnumerationItem.isExternal !== true) {
      ...
      case 'names': {
        if (
          Array.isArray(currentEnumerationItem.names) &&
            ...
            tmpKeys.forEach((namesKey) => {
              namesSub = namesItem.submenu.push({
                ...
                name: `${translationsItemCoreGet(user, 'cmdItemRename')} "${translationsItemTextGet(
                  user,
                  namesKey,
                )}" (${translationsGetEnumName(user, enumerationType, itemIdCurrent, namesKey)})`,
                icon: isCurrentAccessLevelAllowModify ? iconItemEdit : '',
                command: isCurrentAccessLevelAllowModify ? cmdGetInput : cmdNoOperation,
                options: {dataType: enumerationType, item: itemIdCurrent, attribute: 'names', declinationKey: namesKey},
                submenu: [],
              });
            });
      ...
      case 'group': {
        ...
        if (isCurrentAccessLevelAllowModify) {
          groupItem.options = {dataType: enumerationType, item: itemIdCurrent, dataTypeExtraId: enumerationTypeExtraId};
      ...
      case 'holder': {
        if (isCurrentAccessLevelAllowModify && currentEnumerationItem.isExternal) {
          ...
          Object.keys(currentEnumerationsList)
            .filter(
              (itemId) =>
                !itemId.includes('.') && itemId !== itemIdCurrent && currentEnumerationsList[itemId].isEnabled,
            )
            .forEach((itemId) => {
...
function enumerationsIsItemCanBeDeleted(enumerationType, enumerationTypeExtraId, enumerationItemId, withEnum = false) {
  if (
    ...
    const enumeration = enumerationsList[enumerationType],
      enumerationItem =
        typeof enumeration?.list?.[enumerationItemId] === 'object' ? enumeration.list[enumerationItemId] : undefined,
      enumFullId =
...
function enumerationsMenuItemDetailsEnumerationItem(user, menuItemToProcess) {
  ...
  enumerationItemDefaultDetails.forEach((item) => {
    if (currentEnumerationItem.hasOwnProperty(item) || ['itemId', 'name'].includes(item)) {
      ...
      if (typeof currentEnumerationItem[item] === 'boolean') {
        ...
        currentItemDetailsLineObject.value = enumerationsItemName(
          user,
          enumerationType,
          currentItem,
          currentEnumerationsList[currentItem],
        );
...
function enumerationsItemIcon(user, enumerationType, enumerationItemId) {
  ...
  if (isDefined(enumerationList)) {
    ...
    if (typeof enumerationItem?.icon === 'string') {
...
function enumerationsMenuGenerateListOfEnumerationItems(user, menuItemToProcess) {
  ...
  let currentEnumerationsList = enumerationsGetList(enumerationType, enumerationTypeExtraId);
  if (enumerationsDeviceStatesTypes.includes(enumerationType)) {
    if (Object.keys(currentEnumerationsList).length === 0) {
      enumerationsRefreshFunctionDeviceStates(user, enumerationTypeExtraId, enumerationType);
      currentEnumerationsList = enumerationsGetList(enumerationType, enumerationTypeExtraId);
    }
    const currentFunction = enumerationsGetList(enumerationPrimaryType)?.[enumerationTypeExtraId];
    const associatedExtensions = currentFunction?.associatedExtensions || {};
      ...
      if (associatedExtensions[extensionId]?.isEnabled && extensionsList[extensionId]?.isAvailable) {
        ...
        if (typeof attributes === 'object') {
          Object.keys(attributes).forEach((attributeId) => {
            ...
            if (
              (enumerationType === dataTypeDeviceAttributes && attribute?.asAttribute === true) ||
              (enumerationType === dataTypeDeviceButtons && attribute?.asButton === true)
            ) {
              ...
              if (typeof currentEnumerationsList[attributeExternalId] !== 'object') {
                ...
                currentEnumerationsList[attributeExternalId] = {
                  ...
                  stateAttributes: [],
                  order: Object.keys(currentEnumerationsList).length,
                  group: '',
      ...
      if (currentEnumerationItem.isEnabled) {
        ...
        if (enumerationType === dataTypeAssociatedExtensions) {
          if (!extensionsList?.[currentItem]?.isAvailable) {
            ...
            currentIcon = extensionsList?.[currentItem]?.icon || '';
          }
        }
      }
  ...
  switch (enumerationType) {
    ...
    case dataTypeAssociatedExtensions: {
      const associatedExtensions = Object.keys(currentEnumerationsList),
        availableExtensions = Object.keys(extensionsList).filter(
          (extensionId) =>
            extensionsList[extensionId].type === extensionTypeAttributes && !associatedExtensions.includes(extensionId),
        );
      if (availableExtensions.length > 0) {
        ...
        subMenuIndex = subMenu.push(
          menuMenuItemGenerateSelectItem(
            ...
            cmdItemAdd,
            {
              dataType: dataTypeAssociatedExtensions,
              dataTypeExtraId: enumerationTypeExtraId,
              list: currentEnumerationsList,
              mode: 'add',
            },
...
function enumerationsMenuGenerateDevice(user, menuItemToProcess) {
  ...
  if (statesForGraphs.size) {
    ...
    statesForGraphs.forEach((stateName, stateId) => {
      ...
      if (graphsIntervals && Array.isArray(graphsIntervals) && graphsIntervals.length) {
...
function enumerationsExtractPossibleValueStates(user, stateIdOrObject, functionId = '') {
  ...
  if (typeof stateObject.common === 'object') {
    const objectCommon = stateObject.common,
      ...
      stateAttributeId =
        typeof stateFunction === 'object' ? stateId.split('.').slice(-stateFunction.statesSectionsCount).join('.') : '',
    ...
    if (['string', 'object'].includes(`${typeof objectCommon}`)) {
      ...
      for (let iState of statesArray.values()) {
        ...
        } else if (typeof inputStates === 'object' && !Array.isArray(inputStates)) {
          value = iState;
...
function enumerationsTestValueConversionCode(user, functionId, stateToTestOnShortId, convertValueCodeToTest) {
  ...
  $(`state[id=*.${stateToTestOnShortId}](${currentFunction.enum}=${functionId})`).each((stateId) => {
    if (!isStateToTestOnFound) {
      ...
      if (
        currentItemEnums &&
        Array.isArray(currentItemEnums) &&
...
function enumerationsStateValueDetails(user, stateIdOrObject, functionId, currentState, skipCodeConversion = false) {
  const currObject =
    typeof stateIdOrObject === 'string' && existsObject(stateIdOrObject)
      ...
      : stateIdOrObject;
  let text = '';
  if (typeof currObject?._id === 'string' && typeof currObject?.common === 'object') {
    ...
    if (isDefined(currentStateValue)) {
      if (currentStateValueType === 'boolean') {
        ...
        if (states instanceof Map && states.has(currentStateValue)) {
...
function enumerationsMenuItemDetailsDevice(user, menuItemToProcess) {
  ...
  if (typeof externalAttributesProperties === 'object' && typeof externalAttributesProperties.extensions === 'object') {
    const extensions = externalAttributesProperties.extensions,
...
function enumerationsProcessDeviceStatesList(user, currentAccessLevel, options, processStateFn) {
  ...
  Object.keys(deviceStatesList)
    .sort((a, b) => deviceStatesList[a].order - deviceStatesList[b].order)
    .forEach((deviceStateId) => {
      let deviceState = deviceStatesList[deviceStateId],
      ...
      if (deviceStateShortId.length > 0) {
        ...
        if (deviceStateObject?.hasOwnProperty('enumIds')) {
          ...
          if (deviceStateObjectEnums.includes(fullFunctionId) && deviceStateObjectEnums.includes(fullDestinationId)) {
            if (!withDetails) deviceState = {};
            processStateFn(user, deviceStateId, deviceStateIdFull, deviceStateObject, deviceState, options);
          }
        }
      }
    });
            ...
            if (mainObject.enumIds?.includes(fullDestId)) {
              $(`state[id=${idPrefix}.*](${currentFunction.enum}=${functionId})`).each((stateId) => {
                if (existsObject(stateId)) {
                  ...
                  if (currentObject.enumIds?.includes(fullFuncId) && currentObject.enumIds?.includes(fullDestId)) {
                        ...
                        if (currentObjectCommon.hasOwnProperty('type') && currentObjectCommon.type) {
                          ...
                          if (currentDeviceAttrTranslationId) {
                            ...
                            if (currentObjectType === 'boolean') {
                              ...
                              !Array.isArray(currentObjectCommon.states)
                            ) {
...
function alertsGet() {
  if (
    !(typeof alertsRules === 'object' && !Array.isArray(alertsRules) && Object.keys(alertsRules).length > 0) &&
    existsState(alertsStateFullId)
  ) {
    const alertsState = getState(alertsStateFullId);
    if (typeof alertsState === 'object' && !Array.isArray(alertsState) && typeof alertsState.val === 'string') {
      try {
...
function alertsStore(alerts) {
  if (typeof alerts === 'object' && !Array.isArray(alerts) && Object.keys(alerts).length > 0) {
...
function alertsLoadFromCacheStateValue(alertStateId) {
  if (
    !(
      typeof cachedAlertsStatesValues === 'object' &&
      !Array.isArray(cachedAlertsStatesValues) &&
      Object.keys(cachedAlertsStatesValues).length > 0
    ) &&
    ...
    if (
      typeof alertsStatesValuesState === 'object' &&
      !Array.isArray(alertsStatesValuesState) &&
      typeof alertsStatesValuesState.val === 'string'
    ) {
      try {
...
function alertsManage(user, stateId, functionId, destinationId, alertDetailsOrThresholds, isTriggers) {
  ...
  if (!Array.isArray(alertDetailsOrThresholds)) alertDetailsOrThresholds = [];
...
function alertsProcessMessageTemplate(user, template, variables) {
  return template.replace(/\${(.+?)}/g, (_matchedString, stringToProcess) => {
    ...
    if (translations && Array.isArray(translations) && translations.length === 2) {
      ...
      if (ifClause && Array.isArray(ifClause) && ifClause.length === 4 && variables.hasOwnProperty(ifClause[1])) {
...
function alertsActionOnSubscribedState(object) {
  ...
  if (
    typeof alerts?.[stateId] === 'object' &&
    alerts?.[stateId]?.chatIds instanceof Map &&
    alerts?.[stateId]?.chatIds.size > 0
  ) {
          ...
          if (
            ...
            const threshold = detailsOrThresholds[0],
              alertMessageTemplate =
                typeof threshold?.alertMessageTemplateId === 'string'
            ...
            if (threshold?.[onTimeIntervalIndividualId] instanceof Map) {
            ...
            detailsOrThresholds.forEach((threshold) => {
              if (threshold.isEnabled) {
                ...
                const thresholdUsers = threshold.users,
                  targets = Array.isArray(threshold.targets) ? [...threshold.targets] : [];
                targets.unshift(
                  isTrigger
                    ? {
                        isEnabled: true,
...
function alertsHistoryClearOld(user, alertsMessages) {
  const oldestDate = new Date(),
    noInput = !Array.isArray(alertsMessages);
...
function alertsMenuGenerateSubscribed(user, menuItemToProcess) {
  ...
  let alertsListPrepared = {},
    subMenu = [];
  if (typeof alertsList === 'object' && !Array.isArray(alertsList) && Object.keys(alertsList).length) {
...
function alertsMenuItemThresholdDetails(user, menuItemToProcess) {
  ...
  if (timeIntervalsIndividual instanceof Map && timeIntervalsIndividual.size > 0) {
    thresholdAttributesArray.push({
...
function alertsMenuGenerateManageNumericStates(user, menuItemToProcess) {
  ...
  if (isThresholdsSetChanged || (Array.isArray(currentStateThresholds) && currentStateThresholds.length)) {
    subMenuIndex = subMenu.push({
      index: `${currentIndex}.${subMenuIndex}`,
...
function alertsMenuGenerateManageThreshold(user, menuItemToProcess) {
  const currentIndex = typeof menuItemToProcess.index === 'string' ? menuItemToProcess.index : '',
    ...
    stateUnitId =
      typeof menuItemToProcess?.options?.valueOptions?.unit === 'string'
...
function triggersLogsPushTo(triggerLogItem) {
  ...
  if (typeof triggerLogItem === 'object' && !Array.isArray(triggerLogItem)) {
  ...
  if (triggerIndex >= 0) {
    ...
    subMenuIndex = subMenu.push({
      ...
      name:
        ...
        (Array.isArray(conditions) && conditions.length ? ` (${conditions.length})` : ''),
    ...
    if (targetStateId) {
      ...
      subMenuIndex = subMenu.push({
        ...
        name:
          ...
          (Array.isArray(targets) && targets.length ? ` (${targets.length})` : ''),
        icon: iconItemTarget,
      ...
      if (activeUsersList.length) {
        ...
        subMenuIndex = subMenu.push(
          menuMenuItemGenerateSelectItem(user, currentIndex, subMenuIndex, itemName, activeUsers, 'messageTo', {
            ...
            showValueInName: true,
            item: 'users',
            values: Array.isArray(trigger.users) ? trigger.users : [],
            applyMode: true,
            valueOptions: {
      ...
      if (Array.isArray(trigger.users) && trigger.users.length > 0) {
...
function triggersMenuItemDetailsTrigger(user, menuItemToProcess) {
  ...
  if (options && stateId && typeof id === 'string') {
    ...
    if (trigger) {
      ...
      if (Array.isArray(trigger.conditions) && trigger.conditions.length) {
        ...
        trigger.conditions.forEach((condition) => {
          triggerAttributesArray.push({
            label: ` ${triggersGetConditionOrTargetShortDescription(user, condition)}`,
      ...
      if (Array.isArray(trigger.targets) && trigger.targets.length) {
        ...
        trigger.targets.forEach((targets) => {
          triggerAttributesArray.push({
            label: ` ${triggersGetConditionOrTargetShortDescription(user, targets)}`,
            value: menuIconGenerate(user, targets.isEnabled, triggersTargetsIconsArray),
          });
        });
      }
      triggerAttributesArray.push(
  ...
  if (triggerIndex >= 0) {
    ...
    triggersTimeRangeAttributes.forEach((timeRangeAttribute) => {
      const timeRangeAttributePossibleValues = new Map(),
        triggerHasTimeRangeAttribute = Array.isArray(triggerTimeRange[timeRangeAttribute]),
...
function triggerTimeRangeGenerateStartTimes(timeRange) {
  ...
  if (typeof timeRange === 'object') {
    triggersTimeRangeAttributes.forEach((attribute) => {
      if (Array.isArray(timeRange[attribute])) {
            ...
            if (isItFirstAttribute) {
              switch (attribute) {
                ...
                case triggersTimeRangeSeasons: {
                  ...
                  scheduleArray.push(Array.isArray(timeRange[triggersTimeRangeDaysOfWeek]) ? '*' : '1');
                  break;
                }
                case triggersTimeRangeDaysOfWeek: {
                  scheduleArray.push('0', '0', '0', '*', '*');
    ...
    if (triggers.length + triggersOld.length > 0) {
      triggersOld.forEach((triggerOld) => {
        if (triggerOld.isEnabled) {
          ...
          if (Array.isArray(startTimesOld)) {
            startTimesOld.forEach((startTimeOld) => {
              const triggersFiltered = triggers.filter(
                (trigger) =>
                  trigger.isEnabled &&
                  Array.isArray(trigger?.[triggersTimeRangeId]?.[triggerTimeRangeStartTimes]) &&
...
/**
 * This function generates subMenu to manage all Conditions or additional Targets of current Trigger.
 * @param {object} user - The user object.
 * @param {object} menuItemToProcess - The menu item, which will hold newly generated submenu.
 * @returns {object[]} - The array of menuItem objects.
 */
      ...
      items.forEach((itemData) => {
        const {isEnabled} = itemData,
          itemIcon = menuIconGenerate(
            user,
            isEnabled,
            isConditions ? triggersConditionIconsArray : triggersTargetsIconsArray,
          );
        subMenuIndex = subMenu.push({
          index: `${currentIndex}.${subMenuIndex}`,
          name: triggersGetConditionOrTargetShortDescription(user, itemData),
          icon: itemIcon,
          options: {...options, index: subMenuIndex},
          text: triggersMenuItemDetailsConditionOrTarget,
          submenu: triggersMenuGenerateManageConditionOrTarget,
        });
      });
    }
    subMenu.push({
      ...
      icon: iconItemPlus,
      group: cmdItemAdd,
      options: {...options, index: subMenuIndex, subItem: 'state'},
      submenu: triggersMenuGenerateSelectTargetDevice,
    });
  }
  return subMenu;
...
/**
 * This function generates a short descriptions of current Condition or additional Target for the Trigger.
 * @param {object} user - The user object.
 * @param {object} item - The current condition or additional target.
 * @param {boolean=} isCondition - The selector of input data content.
 * @returns {string} - The condition or additional target description.
 */
function triggersGetConditionOrTargetShortDescription(user, item, isCondition = true) {
  let text = '';
  if (item) {
...
/**
 * This function generates the sub Menu with all Conditions or additional Targets for the appropriate Trigger.
 * @param {object} user - The user object.
 * @param {object} menuItemToProcess - The menu item, which will hold newly generated submenu.
 * @returns {object[]} - The array of menuItem objects.
 */
function triggersMenuGenerateManageConditionOrTarget(user, menuItemToProcess) {
  ...
  if (triggerIndex >= 0 && triggersConditionsOrTargets.includes(item)) {
    ...
    if (items && Array.isArray(items) && items.length && index <= items.length) {
      const {
        isEnabled,
        state: stateId,
        function: functionId,
      ...
      subMenuIndex = subMenu.push(
        menuMenuItemGenerateBooleanItem(
          ...
          '',
          {
            ...
            subItem: 'isEnabled',
            value: isEnabled,
            icons: isCondition ? triggersConditionIconsArray : triggersTargetsIconsArray,
          },
        ),
      );
      subMenuIndex = subMenu.push({
...
/**
 * This function generates the text description of the current Condition or additional Target.
 * @param {object} user - The user object.
 * @param {object} menuItemToProcess - The menu item, which will be described.
 * @returns {string} - The description of the Trigger.
 */
function triggersMenuItemDetailsConditionOrTarget(user, menuItemToProcess) {
  const options = menuItemToProcess?.options?.extraOptions
      ? menuItemToProcess.options.extraOptions
      : menuItemToProcess.options,
  ...
  if (options && triggerStateId && typeof id === 'string' && triggersConditionsOrTargets.includes(item)) {
    const triggers = triggersGetStateTriggers(user, triggerStateId),
      triggerIndex = thresholdsGetIndex(triggers, id),
      trigger = triggers && triggerIndex >= 0 ? triggers[triggerIndex] : undefined;
    if (trigger && Array.isArray(trigger[item]) && typeof index === 'number') {
      ...
      if (Array.isArray(items) && index < items.length) {
        const itemData = items[index],
          ...
          itemAttributesArray = [
            {
              label: translationsItemTextGet(user, 'isEnabled'),
              value: menuIconGenerate(
                user,
                isEnabled,
                isCondition ? triggersConditionIconsArray : triggersTargetsIconsArray,
              ),
            },
            {
              label: ` ${translationsItemTextGet(user, 'function')}`,
...
function triggersMenuGenerateSelectTargetDevice(user, menuItemToProcess) {
  const currentIndex = typeof menuItemToProcess.index === 'string' ? menuItemToProcess.index : '',
    ...
    convertDevice = (device) => {
      if (triggersConditionsOrTargets.includes(options.item)) {
        device.function = triggersMenuItemDetailsConditionOrTarget;
      } else {
...
function triggersMenuTriggersLogsItemDetails(user, menuItemToProcess) {
  ...
  if (typeof item === 'object') {
    ...
    if (typeof item.conditions === 'object' && !Array.isArray(item.conditions)) {
      const conditions = item.conditions,
        conditionsChecked = conditions.checked;
      if (
        (typeof conditions.passed === 'boolean' && !conditions.passed) ||
        (Array.isArray(conditionsChecked) && conditionsChecked.length > 0)
      ) {
        triggersLogsItemArray.push({
          label: translationsItemTextGet(user, triggersConditionsId),
      ...
      if (Array.isArray(conditionsChecked) && conditionsChecked.length > 0) {
                ...
                itemsToRestore.forEach((itemToRestore) => {
                  switch (itemToRestore) {
                    case backupItemConfigOptions:
                      if (
                        restoreData.hasOwnProperty(itemToRestore) &&
                        typeof restoreData[itemToRestore] === 'object' &&
                        !Array.isArray(restoreData[itemToRestore])
                      ) {
...
function simpleReportMenuGenerateReportItemsNewSelectStates(user, menuItemToProcess) {
  ...
  if (!destsList.length) {
    destsList =
      typeof destinations === 'object' && !Array.isArray(destinations)
  ...
  if (queryPossibleStates?.length) {
    ...
    queryPossibleStates.forEach(({stateId, destinationId, functionId}, index) => {
      if (destIdCurrent !== destinationId) {
        const holderPrefix =
            typeof destinations[destinationId].holder === 'string'
      ...
      const statesSectionsCount = functions[functionId].statesSectionsCount,
        ...
        iconSelected =
          Array.isArray(queryStates) && queryStates?.includes(stateId)
            ? configOptions.getOption(cfgDefaultIconOn, user)
            : '',
      ...
      if (Array.isArray(queryStates) && queryStates?.includes(stateId)) itemsMarked++;
    });
    if (destIdCurrent) {
      subMenuItem.name += ` (${subMenuItem.submenu.length}, ${itemsMarked})`;
      subMenuIndex = subMenu.push(subMenuItem);
...
function menuMenuItemGenerateRootMenu(user, topRootMenuItemId) {
  ...
  function menuMenuItemGenerateRootMenuSetup(user) {
    ...
    if (
      ...
      subMenu.splice(1, 0, ...enumerationsSubMenuPart);
...
function menuMenuItemGenerateSelectItem(user, upperItemIndex, itemIndex, itemName, itemValues, groupId, options) {
  ...
  if (itemValues && (itemValues instanceof Map || Array.isArray(itemValues))) {
    const itemValuesGroups = itemValues instanceof Map ? [itemValues] : itemValues,
      ...
      convertBeforeApply = inputOptions?.valueOptions?.convertBeforeApply;
    if (applyMode) {
    ...
    if (valueText === iconItemNotFound) {
      if (typeof showInApply === 'function') {
        if (interimCalculationMode === interimCalculationSelectMultiple) {
          if (Array.isArray(valuesPrevious)) valueText = showInApply(valuesPrevious);
      ...
      if (isValueApplicable) {
        if (
          ...
          if (valueCurrent !== undefined) {
            if (typeof convertBeforeApply === 'function') {
              applyOptions.value = convertBeforeApply(valueCurrent);
            } else {
              applyOptions.value = valueCurrent;
            }
          }
          if (valuesCurrent !== undefined) {
            if (typeof convertBeforeApply === 'function') {
              applyOptions.values = convertBeforeApply(valuesCurrent);
            } else {
              applyOptions.values = valuesCurrent;
            }
          }
          if (applyOptions?.valueOptions?.valuesToValue === true) {
            applyOptions.value = applyOptions.values;
            delete applyOptions.values;
          }
          if (typeof applyOptions?.valueOptions?.interimCalculationMode === 'string')
            delete applyOptions.valueOptions.interimCalculationMode;
          if (typeof applyOptions?.valueOptions?.interimItemId === 'string')
            delete applyOptions.valueOptions.interimItemId;
          subMenuIndex = menuItem.submenu.push({
            index: `${currentIndex}.${subMenuIndex}`,
...
function menuMenuItemGenerateEditWeekDays(user, upperItemIndex, itemIndex, itemName, groupId, itemValues, options) {
  const localDaysOfWeek = getLocalDaysOfWeekNames(configOptions.getOption(cfgMenuLanguage, user), 'long'),
    daysOfWeekMap = new Map();
  localDaysOfWeek.forEach((dayOfWeekName, dayOfWeek) => {
    daysOfWeekMap.set(dayOfWeek + 1, dayOfWeekName);
  });
  return menuMenuItemGenerateSelectItem(user, upperItemIndex, itemIndex, itemName, daysOfWeekMap, groupId, {
    ...options,
    applyMode: true,
    values: itemValues,
    replaceCommand: cmdItemSetValue,
    valueOptions: {
      ...options.valueOptions,
      showInApply: (value) => triggerTimeRangeShortDescription(user, {[triggersTimeRangeDaysOfWeek]: value}, true),
      interimCalculationMode: interimCalculationSelectMultiple,
      valuesDefault: triggersTimeRangeAttributesGenerateDefaults(triggersTimeRangeDaysOfWeek),
    },
  });
}
...
function menuMenuItemGenerateEditTime(user, upperItemIndex, itemIndex, itemName, groupId, options) {
  ...
  if (timeTemplate.length) {
    ...
    if (valuesMapArray.length) {
      ...
      if (timeMode === timeInternalModeMonthAndDay || timeMode === timeInternalModeTime) {
        valueType = 'string';
      }
      menuItem = menuMenuItemGenerateEditItemBasedOnValueType(user, upperItemIndex, itemIndex, itemName, groupId, {
        ...options,
...
function menuExtensionsAssignInternalMenuItemsToMenuItem(user, menuItemToProcess) {
  const prepareInternalOptions = (options) => {
    ...
    if (typeof optionsNew.valueOptions?.externalValueId === 'string') {
      delete optionsNew.state;
      delete optionsNew.stateType;
      delete optionsNew.stateObject;
      optionsNew.valueType = optionsNew.valueOptions?.type || optionsNew.valueType;
      optionsNew.dataType = dataTypeExternal;
    ...
    switch (menuItemToProcess.command) {
      case 'editValue': {
        switch (menuItemToProcess.options?.valueOptions?.subType) {
          ...
          case 'weekdays': {
            const weekdays = Array.isArray(menuItemToProcess.options?.value) ? menuItemToProcess.options?.value : [];
...
function menuMenuGenerateFirstLevelAfterRoot(user, menuItemToProcess) {
  ...
  if (primaryMenuItemsList.hasOwnProperty(primaryLevelMenuItemId)) {
    const primaryEnumerationId = isFunctionsFirst ? 'function' : 'destination',
      ...
      deviceList = {},
      indexAhead = menuItemToProcess.indexAhead,
      indexAheadCurrent = indexAhead.split('.').shift();
    ...
    if (
      ...
      Array.isArray(menuItemToProcess.subordinates) &&
     ...
     */
    if (indexAheadCurrent !== indexAhead && secondaryMenuItemsIndex.includes(indexAheadCurrent)) {
      secondaryMenuItemsIndex = [indexAheadCurrent];
    }
    $(`state(${primaryMenuItem.enum}=${primaryLevelMenuItemId})`).each((stateId) => {
      if (existsObject(stateId)) {
        let statesSectionsCount, functionState, shortStateId;
        if (isFunctionsFirst === true) {
          functionState = primaryMenuItem.state;
          statesSectionsCount = primaryMenuItem.statesSectionsCount;
          shortStateId = stateId.split('.').slice(-statesSectionsCount).join('.');
        }
        if (isFunctionsFirst === false || functionState === shortStateId) {
          const currentObject = getObjectEnriched(stateId, '*');
          if (currentObject.hasOwnProperty('enumIds')) {
            secondaryMenuItemsIndex.forEach((currentLevelMenuItemId) => {
              ...
              if (currentObject.enumIds.includes(secondaryFullId)) {
                ...
                if (functionState === shortStateId) {
                  if (deviceList.hasOwnProperty(currentLevelMenuItemId) === false) {
                    deviceList[currentLevelMenuItemId] = [];
                  }
                  if (deviceList[currentLevelMenuItemId].includes(stateId) === false) {
                ...
                if (typeof functionCurrent?.associatedExtensions === 'object') {
                  Object.keys(functionCurrent.associatedExtensions).forEach((extensionId) => {
                    const extension = functionCurrent.associatedExtensions[extensionId],
                      extensionInfo = extensionsList[extensionId] || {};
                    if (extension?.isEnabled && extensionInfo?.isAvailable && extensionInfo?.type === 'attributes') {
  ...
  if (subMenu.length) {
    for (const subMenuIndex of subMenu.keys()) {
      ...
      if (Array.isArray(currentSubMenuItemSubmenu) && currentSubMenuItemSubmenu.length === 0) {
...
class MenuCache {
  ...
  /**
   * This method initializes the internal cache for the appropriate user.
   **/
  init(user) {
    if (isDefined(user?.chatId)) {
      this.#data.set(user.chatId, new Map());
      this.#indexes.set(user.chatId, new Array());
    }
  }

  /**
   * This method sets into the internal cache the menu item and its message for the apropriate menu item.
   * @param {object} user - The user object.
   * @param {object} menuItem - The menu item object.
   * @param {object} message - The message object.
   * @param {string=} indent - The indent for the text part of the message.
   * If the user is not defined, the method will do nothing.
   * If the position is not defined, the method will do nothing.
   **/
  put(user, menuItem, message, indent = '') {
    if (isDefined(user?.chatId)) {
      if (!this.#data.has(user.chatId)) {
        this.init(user);
      }
      if (typeof menuItem === 'object' && typeof menuItem.index) {
        const index = menuItem.index,
          position = index === '' ? [] : index.split('.'),
          userData = this.#data.get(user.chatId),
          indexes = this.#indexes.get(user.chatId);
        if (indexes.includes(index) === false) {
          const indexesFiltered = indexes
            .filter((item) => index.startsWith(item))
            .sort((a, b) => a.localeCompare(b))
            .reverse();
          if (indexesFiltered.length === 0) this.init(user);
          indexes.push(index);
        } else {
          this.clearFrom(user, position);
        }
        userData.set(index, objectDeepClone({position, menuItem, message, indent}));
        this.#data.set(user.chatId, userData);
        this.#indexes.set(user.chatId, indexes);
      }
    }
  }

  /**
   * This method gets the menu item and its message for the apropriate menu item.
   * @param {object} user - The user object.
   * @param {string[]} position - The position of the menu item in the menu tree.
   * @returns {object} The object with the menu item and its message.
   * If the user is not defined, the default object will be returned.
   * If the position is not defined, the default object will be returned.
   **/
  get(user, position) {
    let result = {position, indent: ''};
    if (isDefined(user?.chatId)) {
      if (this.#data.has(user.chatId)) {
        ...
        if (indexes.includes(index)) {
          ...
          const indexesFiltered = indexes
            ...
            .sort((a, b) => a.localeCompare(b))
            .reverse();
          if (indexesFiltered.length > 0) {
            result = userData.get(indexesFiltered[0]);
          } else {
            this.init(user);
          }
        }
   ...
   * If the `position` is not defined, the method will do nothing.
   **/
  clearFrom(user, position) {
    if (isDefined(user?.chatId)) {
      if (Array.isArray(position) && this.#data.has(user.chatId)) {
        const index = position.join('.'),
          userData = this.#data.get(user.chatId),
          indexes = this.#indexes.get(user.chatId);
        indexes.forEach((item) => {
          if (item.startsWith(index)) {
            userData.delete(item);
            indexes.splice(indexes.indexOf(item), 1);
          }
        });
        this.#data.set(user.chatId, userData);
        this.#indexes.set(user.chatId, indexes);
 ...
 * @param {string[]=} position - The position of the menu item in the menu tree
 */
function menuCacheClearAt(user, position) {
  if (Array.isArray(position) === false) {
    position = cachedValueGet(user, cachedMenuItem);
  }
  menuCache.clearFrom(user, position);
}
...
function menuMenuDraw(user, targetMenuPos, messageOptions, menuItemToProcess, messageObject, currentIndent = '') {
   ...
   */
  if (!messageObject) {
    const cached = Array.isArray(targetMenuPos) ? menuCache.get(user, targetMenuPos) : {};
    if (Array.isArray(targetMenuPos) && typeof cached.menuItem === 'object' && typeof cached.message === 'object') {
      menuItemToProcess = cached.menuItem;
      targetMenuPos = targetMenuPos.slice(cached.position.length);
      messageObject = cached.message;
  ...
  menuItemToProcess.indexAhead = Array.isArray(targetMenuPos) ? targetMenuPos.join('.') : '';
  switch (typeof subMenu) {
    case 'string': {
      const extensionMenuId = subMenu,
        extensionId = menuItemToProcess.extensionId;
      if (extensionsList?.[extensionId]?.isAvailable === true) {
        const data = objectDeepClone(menuItemToProcess),
          externalValueId = data?.options?.valueOptions?.externalValueId || data.index;
        if (typeof externalValueId === 'string' && cachedValueExists(user, cachedInterimValue)) {
          const interimValues = cachedValueGet(user, cachedInterimValue);
          if (interimValues?.[externalValueId] !== undefined) {
          ...
          (newMenuItem) => {
            if (newMenuItem?.error === undefined && typeof newMenuItem?.id === 'string' && newMenuItem?.id.length > 0) {
              ...
              if (typeof menuItemToProcess.goTo === 'string' && menuItemToProcess.goTo.length > 0) {
                ...
                menuCacheClearAt(user, menuItemToProcess.goTo.split('.'));
                ...
                if (extensionsList?.[extensionId]?.type === extensionTypeAttributes) {
                  if (
                    ...
                    interimValues[externalValueId] = objectDeepClone(newMenuItem.options.valueOptions.externalValue);
                    cachedValueSet(user, cachedInterimValue, interimValues);
                  }
              ...
              menuMenuDraw(user, menuItemToProcess.index.split('.').slice(0, -1));
            }
          },
        );
      } else {
        warns(`Extension menu ${extensionMenuId} is not available!`);
        menuMenuDraw(user, menuItemToProcess.index.split('.').slice(0, -1));
      }
    ...
    default: {
      ...
      if (typeof subMenuPos === 'number' && subMenuPos >= 0 && subMenu.length > 0 && subMenuPos < subMenu.length) {
        ...
        messageObject.options = typeof subMenuItem?.options === 'object' ? subMenuItem.options : {};
        ...
        const externalAttributes = menuItemToProcess.options.externalAttributes,
          ...
          extensionId = externalAttributeId !== undefined ? externalAttributes?.[externalAttributeId]?.extensionId : '',
        ...
        Object.keys(menuItemToProcess.options.externalAttributesProperties.extensions).every(
          (extensionId) =>
            typeof menuItemToProcess.options.externalAttributesProperties.extensions[extensionId]?.['valueText'] !==
            'object',
        )
      ) {
        const externalAttributesProperties = menuItemToProcess.options.externalAttributesProperties,
          extensionId = Object.keys(externalAttributesProperties.extensions).find(
            (extensionId) =>
              typeof menuItemToProcess.options.externalAttributesProperties[extensionId]?.valueText !== 'object',
          );
        ...
        menuCache.put(user, menuItemToProcess, {...messageObject}, currentIndent);
        messageObject.message += messageCaption + menuMenuItemGetIcon(user, menuItemToProcess) + menuItemToProcess.name;
        ...
        if (buttonsOffset > 0) {
          ...
          if (commandOptionsCache.error) {
            warns(
              `Can't set command options for ${[indexCurrent, 'prev'].join('.')}!, error is ${
                commandOptionsCache.error
              }.`,
            );
          } else {
            messageObject.buttons.push({
        ...
        if (buttonsCount > buttonsCountMax) {
          ...
          if (commandOptionsCache.error) {
            warns(
              `Can't set command options for ${[indexCurrent, 'next'].join('.')}!, error is ${
                commandOptionsCache.error
              }.`,
            );
          } else {
        ...
        if (isDefined(messageObject.navigationLeft)) {
          ...
          if (commandOptionsCache.error) {
            warns(
              `Can't set command options for ${[indexCurrent, 'left'].join('.')}!, error is ${
                commandOptionsCache.error
              }.`,
            );
          } else {
...
function menuMenuItemGetIcon(user, menuItemToProcess) {
  let icon = '';
  if (typeof menuItemToProcess === 'object' && !Array.isArray(menuItemToProcess)) {
    if (menuItemToProcess.hasOwnProperty('options')) {
      const options = menuItemToProcess.options || {},
        {function: currentFunctionId, state: currentStateId, device: devicePrefix} = options,
        currentFunction =
          currentFunctionId && enumerationsList[dataTypeFunction].list.hasOwnProperty(currentFunctionId)
      ...
      if (icon !== iconItemUnavailable) {
        if (typeof menuItemToProcess.icons === 'object' && !Array.isArray(menuItemToProcess.icons)) {
...
function menuMenuUpdateBySchedule() {
  ...
  logs(`try to refresh menu for users ${usersIds}`);
  for (const userId of usersIds) {
    const user = telegramGenerateUserObjectFromId(userId);
    if (cachedValueGet(user, cachedMenuOn) === true) {
      const itemPos = cachedValueGet(user, cachedMenuItem);
      logs(`for user = ${jsonStringify(userId)} menu on pos ${jsonStringify(itemPos)}`);
...
function menuMenuReIndex(inputMenu, indexPrefix, extraOptions) {
  ...
  if (Array.isArray(inputMenu)) {
      ...
      .forEach((menuItem, menuItemIndex) => {
        ...
        Object.keys(menuItem).forEach((attributeId) => {
          switch (attributeId) {
            ...
            case 'submenu': {
              if (Array.isArray(menuItem.submenu) && menuItem.submenu.length) {
  ...
  } else if (inputMenu?.hasOwnProperty('submenu')) {
    ...
    newMenuRow.submenu = Array.isArray(inputMenu.submenu)
 ...
 * @param {object} user - The user object.
 * @param {string} userInputValue - The string, contained the user input.
 */
async function commandsUserInputProcess(user, userInputValue) {
  let timer, userInputToProcess;
  userInputToProcess = userInputValue;
  ...
  /**
   * This function is used to process setting the interim values.
   * @param {object} commandOptions - The command options object.
   * @param {any=} userInputValue - The possible value for the command.
   **/
  function setInterimValue(commandOptions, userInputValue) {
    if (typeof commandOptions.valueOptions?.interimItemId === 'string') {
      const interimItemId = commandOptions?.valueOptions?.interimItemId,
        interimCalculationMode =
          typeof commandOptions?.valueOptions?.interimCalculationMode === 'string'
      ...
      if (typeof interimItemId === 'string' && typeof interimCalculationMode === 'string') {
        switch (interimCalculationMode) {
          case interimCalculationSummarize: {
            if (inputValue !== undefined) {
              const valueCurrent = commandOptions.valueOptions?.valueCurrent,
                modificator = isDefined(inputValue) ? Number(inputValue) : 0;
              switch (commandOptions.valueOptions?.subType) {
                case 'time': {
                ...
                default: {
                  const valueBase = typeof valueCurrent === 'number' ? valueCurrent : 0,
                    ...
                    valueModifier = typeof inputValue === 'number' ? inputValue : 0,
          ...
          case interimCalculationSelectMultiple: {
            if (
              commandOptions?.valueOptions?.resetValue === true &&
              isDefined(commandOptions?.valueOptions?.valuesDefault)
            ) {
              interimValue = commandOptions.valueOptions.valuesDefault;
            } else if (isDefined(commandOptions.values) && isDefined(inputValue)) {
              interimValue = [...commandOptions.values];
              if (interimValue.includes(inputValue)) {
                interimValue = interimValue.filter((item) => item !== inputValue);
              } else {
                interimValue.push(inputValue);
                if (typeof inputValue === 'number') interimValue = interimValue.sort((a, b) => a - b);
              }
            }
            break;
          }
          ...
          default: {
            if (inputValue !== undefined)
              interimValue = commandOptions.valueType === 'number' ? Number(inputValue) : inputValue;
            break;
          }
        }
      }
      if (isDefined(interimValue)) {
        const interimValues = cachedValueGet(user, cachedInterimValue) || {};
        interimValues[interimItemId] = interimValue;
        cachedValueSet(user, cachedInterimValue, interimValues);
  ...
  if (isWaitForInput || commandCurrent === cmdItemSetValue) {
    if (userInputToProcess != dataTypeIgnoreInput) {
      switch (commandCurrent) {
        ...
        case cmdGetInput:
        default: {
          if (commandOptions?.valueType === 'number' || commandOptions?.valueOptions?.type === 'number') {
            userInputToProcess = Number(userInputToProcess);
            if (isNaN(userInputToProcess)) {
              userInputToProcess = undefined;
            } else {
              if (typeof commandOptions?.valueOptions?.step === 'number' && commandOptions?.valueOptions?.step > 0) {
                const step = commandOptions?.valueOptions?.step;
                if (userInputToProcess % step !== 0) {
                  userInputToProcess = undefined;
                }
              }
              if (userInputToProcess !== undefined && typeof commandOptions?.valueOptions?.min === 'number') {
                const min = commandOptions?.valueOptions?.min;
                if (userInputToProcess < min) {
                  userInputToProcess = undefined;
                }
              }
              if (userInputToProcess !== undefined && typeof commandOptions?.valueOptions?.max === 'number') {
                const max = commandOptions?.valueOptions?.max;
                if (userInputToProcess > max) {
                  userInputToProcess = undefined;
                }
              }
            }
          }
          if (userInputToProcess === undefined) {
            telegramMessageDisplayPopUp(user, translationsItemTextGet(user, 'MsgValueUnacceptable'));
          } else if (
            typeof commandOptions?.valueOptions?.interimItemId === 'string' &&
            typeof commandOptions?.valueOptions?.interimCalculationMode === 'string' &&
            commandOptions?.applyMode === true
          ) {
            setInterimValue(commandOptions, userInputToProcess);
          } else {
            switch (commandOptions.dataType) {
              case dataTypeTranslation: {
                ...
                if (
                  ...
                  let destTranslation = translationsPointOnItemOwner(
                    user,
                    `${currentTranslationId}.destinations`,
                    true,
                  );
                  if (
                    destTranslation &&
                ...
                menuCacheClearAt(user, menuPositionCurrent.slice(0, -1));
                break;
              }
              ...
              case dataTypeDeviceButtons: {
                ...
                enumerationsSave(
                  enumerationsGetPrimaryDataType(commandOptions.dataType, commandOptions.dataTypeExtraId),
                );
                break;
              }
                        ...
                        const currentMask = configOptions.getMask(commandOptions.item),
                          ...
                          parsedValue =
                            ...
                            timeIntervalsInMinutes.hasOwnProperty(parsedValueArray[2])
                              ...
                              : undefined;
                        newValue =
                          typeof parsedValue === 'number' ? {id: userInputToProcess, minutes: parsedValue} : null;
                  ...
                  switch (mode) {
                    case 'add': {
                      if (commandOptions.id === thresholdEnumerable) {
                        ...
                        if (thresholdsGetIndex(detailsOrThresholds, thresholdId) >= 0) {
                          ...
                          if (!Array.isArray(detailsOrThresholds)) detailsOrThresholds = new Array();
                          detailsOrThresholds.push({
                            isEnabled: true,
                            id: thresholdId,
                            index: 0,
                    ...
                    case 'edit': {
                      const thresholdId = commandOptions.id,
                        thresholdIndex =
                          typeof thresholdId === 'string' ? thresholdsGetIndex(detailsOrThresholds, thresholdId) : -1;
                      if (thresholdIndex >= 0) {
                          ...
                          case onTimeIntervalIndividualId: {
                            if (Number.isNaN(userInputToProcess)) {
                              ...
                              if (item === onTimeIntervalId) {
                                ...
                                if (intervalsIndividual instanceof Map && intervalsIndividual.size > 0) {
                                ...
                                if (thresholdValue !== timeIntervalCommon) {
                                  if (!(threshold[item] instanceof Map)) threshold[item] = new Map();
                              ...
                              if (
                                ...
                                (!(threshold[onTimeIntervalIndividualId] instanceof Map) ||
              ...
              case dataTypeTrigger: {
                if (commandOptions.state) {
                  let triggers = triggersGetStateTriggers(
                      user,
                      typeof commandOptions.triggerState === 'string'
                        ? commandOptions.triggerState
                        : commandOptions.state,
                    ),
                    backStepsForCacheDelete = -1;
                  if (!Array.isArray(triggers)) triggers = new Array();
                  switch (commandOptions.mode) {
                    case 'add': {
                              ...
                              if (Number.isNaN(userInputToProcess)) {
                                ...
                                if (item === 'value') {
                                  if (!checkNumberStateValue(commandOptions.state, triggerValue)) {
                                    ...
                                    const triggerIdPrefix = [
                                      triggerValue,
                                      trigger.onAbove ? 'onAbove' : 'onBelow',
                                    ].join('#');
                                    trigger.index = triggersGetMaxSubIndex(triggers, `${triggerIdPrefix}#`) + 1;
                            ...
                            case triggersConditionsId: {
                              ...
                              if (Array.isArray(items) && typeof index === 'number' && subItem) {
                                ...
                                if (index < length) {
                                  ...
                                  switch (subItem) {
                                    case 'value': {
                                      const itemStateObject = commandOptions.stateObject,
                                        itemStateObjectCommon = itemStateObject?.common,
                                        itemStateType = itemStateObjectCommon?.type;
                                      let itemValue;
                                      if (itemStateType === 'number') {
                                        if (Number.isNaN(userInputToProcess)) {
                                          warns(`Unacceptable value '${userInputToProcess}' for number conditions`);
                                          telegramMessageDisplayPopUp(
                                            user,
                                          ...
                                          itemValue = Number(userInputToProcess);
                                          if (!checkNumberStateValue(trigger.targetState, itemValue, itemStateObject)) {
                                            warns(`Unacceptable value '${itemValue}' for state conditions`);
                                            telegramMessageDisplayPopUp(
                                              user,
                                              translationsItemTextGet(user, 'MsgValueUnacceptable'),
                                            );
                                        ...
                                        itemValue = userInputToProcess;
                                      }
                                      if (triggers) itemData[subItem] = itemValue;
                                      break;
                                    }
                    ...
                    if (valueInterim !== undefined) {
                      switch (externalValueType) {
                        case 'array#object': {
                          ...
                          if (index >= 0 && typeof commandOptions?.valueOptions?.externalObjectTemplate === 'object') {
    ...
    switch (commandCurrent) {
      ...
      case cmdItemSetInterimValue: {
        setInterimValue(commandOptions);
        break;
      }
      ...
      case cmdItemPress: {
        switch (commandOptions.dataType) {
            ...
            if (typeof currentEnumerationsList === 'object') {
              if (typeof commandOptions.item === 'string' && currentEnumerationsList?.[commandOptions.item]) {
                ...
                if (typeof commandOptions.attribute === 'string') {
                  if (
                    ...
                    if (
                      ...
                      const currentDeviceAttribute = enumerationsGetDeviceState(enumerationType, enumerationExtraId),
                        currentSort = (a, b) =>
                          enumerationsCompareOrderOfItems(
                            a,
                            b,
                            dataTypeDeviceStatesAttributes,
                ...
                if (typeof commandOptions.value === 'string' && commandOptions.mode === 'add') {
                  ...
                  enumerationsSave(enumerationsGetPrimaryDataType(enumerationType, enumerationExtraId));
                  menuPositionCurrent.splice(-1, 1, Object.keys(currentEnumerationsList).indexOf(extensionId));
                }
              }
            }
            break;
          ...
          case dataTypeTrigger: {
            if (commandOptions.state) {
              ...
              switch (mode) {
                case 'save': {
                  if (commandOptions.function && commandOptions.destination && Array.isArray(triggers)) {
                    ...
                    if (triggerIndex >= 0) {
                      ...
                      switch (triggerAttributeId) {
                        ...
                        case triggersTimeRangeId: {
                          ...
                          switch (triggerTimeRangeAttributeId) {
                            ...
                            case triggersTimeRangeMonthsWithDays: {
                              if (typeof commandOptions.index === 'number') {
                                ...
                                } else if (Array.isArray(commandOptions.value)) {
                                  const subItemWasBlank = !Array.isArray(triggerTimeRange[triggerTimeRangeAttributeId]);
                                  backStepsForCacheDelete--;
                          ...
                          if (Array.isArray(triggers) && Object.keys(triggerTimeRange).length === 0) {
                            delete trigger[triggerAttributeId];
                          } else {
                        ...
                        case triggersConditionsId: {
                          ...
                          if (typeof index === 'number' && typeof subItem === 'string' && index <= length) {
                            if (!Array.isArray(trigger[triggerAttributeId])) trigger[triggerAttributeId] = items;
                            backStepsForCacheDelete -= 2;
        ...
        menuCacheClearAt(user, menuPositionCurrent);
        break;
      }
                ...
                if (triggerIndex >= 0) {
                  ...
                  if (typeof triggerAttributeId === 'string') {
                    ...
                    switch (triggerAttributeId) {
                      case triggersTimeRangeId: {
                        if (
                          typeof trigger[triggerAttributeId] === 'object' &&
                          !Array.isArray(trigger[triggerAttributeId])
                        ) {
                          const triggerTimeRange = trigger[triggerAttributeId],
                            triggerTimeRangeAttributeId = commandOptions.subItem,
                            triggerTimeRangeAttribute = triggerTimeRange[triggerTimeRangeAttributeId];
                          delete triggerTimeRange[triggerTimeRangeStartTimes];
                          if (Array.isArray(triggerTimeRangeAttribute)) {
            ...
            if (valueInterim !== undefined || subMode === 'boolean') {
              switch (commandOptions?.valueOptions?.externalValueType) {
                case 'array#object': {
                  ...
                  if (index !== undefined && index >= 0 && index < valueInterim.length) {
                    valueInterim.splice(index, 1);
                    cachedValueSet(user, cachedInterimValue, interimValues);
                    menuPositionCurrent.splice(-2, 2);
                  }
                  break;
        ...
        switch (commandOptions.dataType) {
          ...
          case dataTypeReportMember: {
            ...
            switch (commandOptions.itemType) {
              ...
              case 'states':
                if (
                  Array.isArray(queryParams.queryStates) &&
          ...
          case dataTypeAlertSubscribed: {
            if (
              ...
              if (alertIsOn) {
                ...
                if (Array.isArray(alertDetailsSource) && alertDetailsSource.length > 0) {
                              ...
                              currentEnumsList
                                ...
                                .every((enumId) => {
                                  const enumFullId = [prefixEnums, enumId, ''].join('.'),
                                    enumIndex = currentStateEnumIds.findIndex((enumId) =>
                                      enumId.startsWith(enumFullId),
                                    );
                                  if (enumIndex >= 0) {
                        ...
                        if (toProcessState) {
                          ...
                          if (
                            ...
                            (propagateMode === 'alertPropagateOverwrite' ||
                              !Array.isArray(alertDetailsCurrent) ||
    ...
    if (Array.isArray(menuPositionCurrent)) menuMenuDraw(user, menuPositionCurrent);
  }
}
...
const telegramMessageQueue = new Map(),
  ...
  if (isMenuOn || createNewMessage) {
    ...
    if (lastMessagePlainText !== currentMessagePlainText || createNewMessage || clearBefore) {
      ...
      if ((createNewMessage && !isMenuOn) || clearBefore || isBotMessageOldOrNotExists) {
        if (Array.isArray(messageObject.buttons)) {
...
function telegramObjectPushToQueue(user, telegramObject) {
  if (telegramMessageQueue.has(user?.chatId) === false) {
    telegramMessageQueue.set(user?.chatId, new Array());
  }
  const queueOfUser = telegramMessageQueue.get(user?.chatId);
  const isReady = queueOfUser.length === 0;
  if (Array.isArray(telegramObject) && telegramObject.length && Array.isArray(telegramObject[0])) {
    telegramObject.forEach((telegramSubObject) => queueOfUser.push(telegramSubObject));
  } else {
    queueOfUser.push(telegramObject);
  }
  if (isReady) {
...
function telegramQueueProcess(user, messageId) {
  ...
  function telegramSendToCallBack(result, user, telegramObject, telegramObjects, sendToTS, waitForLog = false) {
      ...
      if (waitForLog) {
        ...
        if (telegramLastErrors?.size) {
          if (user.chatId !== 0 && telegramLastErrors.has(user.chatId)) {
            ...
            if (Array.isArray(groupChatErrors) && groupChatErrors.length) {
        ...
        if (resultObject.error?.level === telegramErrorLevelFatal) {
          warns(`Going to retry send the whole queue after timeout = ${telegramDelayToSendReTry} ms.`);
          telegramObject = undefined;
          if (telegramQueuesIsWaitingConnection.find((queueUser) => queueUser.chatId === user.chatId) === undefined) {
            telegramQueuesIsWaitingConnection.push(user);
          }
          setTimeout(() => {
            warns(`Retrying message send for chat ${user.chatId}.`);
            const indexWainting = telegramQueuesIsWaitingConnection.findIndex(
              (queueUser) => queueUser.chatId === user.chatId,
            );
            if (indexWainting >= 0) {
    ...
    if (resultObject.success) {
      ...
      } else if (typeof telegramObject?.[telegramCommandDeleteMessage]?.isBotMessage === 'number') {
        cachedValueSet(user, cachedBotSendMessageId, 0);
      } else {
        cachedValueSet(user, cachedBotSendMessageId, resultObject.messageId);
      }
    }
    if (telegramObjects.length) {
      ...
      const queueOfUser = telegramMessageQueue.get(user?.chatId);
      if (Array.isArray(queueOfUser) && queueOfUser.length > 0) {
        queueOfUser.shift();
      }
      telegramMessageQueueIsProcessing.delete(user.chatId);
      if (Array.isArray(queueOfUser) && queueOfUser.length > 0) {
  ...
  if (
    ...
    const queueOfUser = telegramMessageQueue.get(user?.chatId);
    if (Array.isArray(queueOfUser) && queueOfUser.length) {
       ...
       */
      if (queueOfUser.length > 2) {
        queueOfUser.splice(0, queueOfUser.length - 1);
      }
      let telegramObjects;
      do {
        telegramObjects = objectDeepClone(queueOfUser[0]);
      ...
      } while (Array.isArray(telegramObjects) === false && queueOfUser.length > 0);
...
const telegramQueuesIsWaitingConnection = [],
  ...
  telegramErrorParseRegExp =
    /^telegram.\d\s+\(\d+\)\s+(?<result>Cannot\s+send|Failed)\s+(?<operation>\w+)\s+\[(?<target>chatId|user)\s-\s(?<id>-?\d+|\w+)\]:\s+Error:\s(?<level>\w+?):\s(?<errorInfo>.+?):\s(?<errorDetails>.+)$/, // NOSONAR
...
function telegramActionOnLogError(logRecord) {
  if (logRecord?.from === telegramAdapter) {
    try {
      const telegramErrorParsed = telegramErrorParseRegExp.exec(logRecord.message)?.groups;
      if (typeof telegramErrorParsed === 'object') {
        let chatId;
        try {
          // @ts-ignore
          chatId = isNaN(telegramErrorParsed.id) ? 0 : Number(telegramErrorParsed.id);
        } catch (error) {
          logs(`Can't parse chatId from error message: ${jsonStringify(error, 1)}`, _l);
          chatId = 0;
        }
        const errorMessage = {
          ts: logRecord.ts,
          chatId: chatId,
          error: {
            command: telegramErrorParsed.operation,
            level: telegramErrorParsed.level,
            info: telegramErrorParsed.errorInfo,
            message: telegramErrorParsed.errorDetails,
          },
        };
        if (chatId) {
      ...
      warns(`Can't parse log record: ${jsonStringify(logRecord, 1)}! Error: ${jsonStringify(error, 1)}.`);
    }
  }
}
        ...
        if (typeof chatId === 'number') {
          ...
          if (isDefined(command)) {
            if (userRequest?.data) {
              ...
              const queueOfUser = telegramMessageQueue.get(user.chatId);
              if (Array.isArray(queueOfUser) && queueOfUser.length > 0) {
                warns(`Some output is in cache:\n${jsonStringify(queueOfUser)}.\nGoing to delete it!`);
                queueOfUser.splice(0, queueOfUser.length);
                if (telegramMessageQueueIsProcessing.has(user.chatId)) {
                  telegramMessageQueueIsProcessing.delete(user.chatId);
                }
                menuCacheClearAt(user);
...
function telegramActionOnSendToUserRaw(obj) {
  ...
  if (botMessage?.message_id && typeof botMessage?.chat?.type == 'string') {
    let user = {};
...
function telegramActionOnConnected(connected) {
  function telegramQueueProcessWaiting() {
    if (telegramQueuesIsWaitingConnection.length) {
      setTimeout(() => {
        telegramQueueProcess(telegramQueuesIsWaitingConnection.shift());
        if (telegramQueuesIsWaitingConnection.length) {
          telegramQueueProcessWaiting();
        }
      }, Math.round(telegramDelayToSendReTry / 10));
    }
  }
  if (connected?.state?.hasOwnProperty('val')) {
  ...
  if (telegramIsConnected) {
    logs(`Telegram adapter is connected!`, true);
    /** answerRawSubscribe */
    ...
    if (telegramQueuesIsWaitingConnection.length) {
      warns(`Going to resend messages for ${telegramQueuesIsWaitingConnection.length} users/chats!`);
      telegramQueueProcessWaiting();
    }
  }
}
...
function objectDeepClone(source, filter = {}) {
  ...
  if (source instanceof Map) {
    ...
    source.forEach((value, key) => {
      if (!filter.hasOwnProperty(key) || (typeof filter[key] === 'object' && !Array.isArray(filter[key]))) {
  ...
  for (const key in source) {
    if (
      ...
      (!filter.hasOwnProperty(key) || (typeof filter[key] === 'object' && !Array.isArray(filter[key])))
    ) {
...
function objectKeysSort(inputObject) {
  ...
  if (typeof inputObject === 'object' && !Array.isArray(inputObject)) {
    ...
    keys.forEach((id) => {
      ...
      if (typeof inputObject[id] === 'object' && !Array.isArray(inputObject[id])) {
